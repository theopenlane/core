...
func (_q *OrgProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrgProduct, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, orgproduct.ForeignKeys...)
	}
...
func (_q *OrgProductQuery) loadOrgPrices(ctx context.Context, query *OrgPriceQuery, nodes []*OrgProduct, init func(*OrgProduct), assign func(*OrgProduct, *OrgPrice)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Select(joinT.C(orgproduct.OrgPricesPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*OrgProduct]struct{}{byID[outValue]: {}}
...
func (_q *OrgProductQuery) loadOrgModules(ctx context.Context, query *OrgModuleQuery, nodes []*OrgProduct, init func(*OrgProduct), assign func(*OrgProduct, *OrgModule)) error {
	...
	query.Where(predicate.OrgModule(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(orgproduct.OrgModulesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.org_product_org_modules
		if fk == nil {
			return fmt.Errorf(`foreign-key "org_product_org_modules" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "org_product_org_modules" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
