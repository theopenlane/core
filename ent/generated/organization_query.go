...
// QueryTrustCenterSubprocessorCreators chains the current query on the "trust_center_subprocessor_creators" edge.
func (_q *OrganizationQuery) QueryTrustCenterSubprocessorCreators() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
// WithTrustCenterSubprocessorCreators tells the query-builder to eager-load the nodes that are connected to
// the "trust_center_subprocessor_creators" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithTrustCenterSubprocessorCreators(opts ...func(*GroupQuery)) *OrganizationQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withTrustCenterDocCreators; query != nil {
		if err := _q.loadTrustCenterDocCreators(ctx, query, nodes,
			func(n *Organization) { n.Edges.TrustCenterDocCreators = []*Group{} },
			func(n *Organization, e *Group) {
				n.Edges.TrustCenterDocCreators = append(n.Edges.TrustCenterDocCreators, e)
			}); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedTrustCenterSubprocessorCreators {
		if err := _q.loadTrustCenterSubprocessorCreators(ctx, query, nodes,
...
func (_q *OrganizationQuery) loadTrustCenterDocCreators(ctx context.Context, query *GroupQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Group)) error {
	...
	query.withFKs = true
	query.Where(predicate.Group(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.TrustCenterDocCreatorsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.organization_trust_center_doc_creators
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_trust_center_doc_creators" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *OrganizationQuery) loadTrustCenterSubprocessorCreators(ctx context.Context, query *GroupQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Group)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Group(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.TrustCenterSubprocessorCreatorsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.organization_trust_center_subprocessor_creators
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_trust_center_subprocessor_creators" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
// WithNamedTrustCenterSubprocessorCreators tells the query-builder to eager-load the nodes that are connected to the "trust_center_subprocessor_creators"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
