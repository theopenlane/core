...
func HandleUploads(ctx context.Context, svc *objstore.Service, files []objects.File) (context.Context, []objects.File, error) {
	if len(files) == 0 {
		return ctx, nil, nil
	}

	var uploadedFiles []objects.File
	...
	for _, file := range files {
		...
		if uploadOpts.ProviderHints == nil {
			uploadOpts.ProviderHints = &objects.ProviderHints{}
		}
	...
	contextFilesMap := make(objects.Files)
	...
	ctx = objects.WriteFilesToContext(ctx, contextFilesMap)
...
func HandleRollback(ctx context.Context, svc *objstore.Service, files []objects.File) {
	if len(files) == 0 {
		return
	}
...
func BuildUploadOptions(ctx context.Context, f *objects.File) *objects.UploadOptions {
	if f == nil {
		return &objects.UploadOptions{}
	}

	if f.ProviderHints == nil {
		f.ProviderHints = &objects.ProviderHints{}
	}

	orgID, _ := auth.GetOrganizationIDFromContext(ctx)
	objstore.PopulateProviderHints(f, orgID)
	...
	if contentType == "" || strings.EqualFold(contentType, "application/octet-stream") {
		...
		if f.RawFile != nil {
			...
			} else if buffered, err := objects.NewBufferedReaderFromReader(f.RawFile); err == nil {
...
func mergeUploadedFileMetadata(dest *objects.File, entFileID string, src objects.File) {
	dest.ID = entFileID
