...
// PropertyView provides typed helpers for accessing and mutating event properties.
type PropertyView struct {
	props Properties
}

// NewPropertyView wraps the provided properties map with typed helpers.
func NewPropertyView(props Properties) PropertyView {
	if props == nil {
		props = NewProperties()
	}

	return PropertyView{props: props}
}

// Raw exposes the underlying properties map.
func (v PropertyView) Raw() Properties {
	return v.props
}

// Has reports whether a property exists.
func (v PropertyView) Has(key string) bool {
	_, ok := v.props[key]
	return ok
}

// Get retrieves a property by key.
func (v PropertyView) Get(key string) (any, bool) {
	if v.props == nil {
		return nil, false
	}

	value, ok := v.props[key]
	if !ok || value == nil {
		return nil, false
	}
...
// String fetches a string property.
func (v PropertyView) String(key string) (string, bool) {
	value, ok := v.Get(key)
	if !ok {
		return "", false
	}
...
// Set mutates or adds a property.
func (v PropertyView) Set(key string, value any) {
	v.props.Set(key, value)
}

// Delete removes a property from the map.
func (v PropertyView) Delete(key string) {
	if v.props == nil {
		return
	}
