...
func TestEmitDerivesTopicUniqueKey(t *testing.T) {
	...
	if _, err := runtime.EmitWithHeaders(context.Background(), topic.Name, runtimeTestPayload{Message: "loop-a"}, Headers{Kind: System.Kind()}); err != nil {
...
func TestEmitRawPayloadDerivesTopicUniqueKey(t *testing.T) {
	...
	if _, err := runtime.EmitWithHeaders(context.Background(), topic.Name, nil, Headers{Kind: System.Kind()}, WithRawPayload(encodedPayload)); err != nil {
...
func TestEmitSkipUniqueKeySuppressesDerivation(t *testing.T) {
	...
	if _, err := runtime.EmitWithHeaders(context.Background(), topic.Name, runtimeTestPayload{Message: "loop-a"}, Headers{SkipUniqueKey: true, Kind: System.Kind()}); err != nil {
		t.Fatalf("unexpected emit error: %v", err)
	}
...
func TestEmitExplicitUniqueKeyWins(t *testing.T) {
	...
	if _, err := runtime.EmitWithHeaders(context.Background(), topic.Name, runtimeTestPayload{Message: "loop-a"}, Headers{UniqueKey: "explicit", Kind: System.Kind()}); err != nil {
		t.Fatalf("unexpected emit error: %v", err)
	}
...
func TestUniqueKeyEnforcedByRiver(t *testing.T) {
	...
	for range 2 {
		if _, err := runtime.EmitWithHeaders(ctx, topic.Name, runtimeTestPayload{Message: "loop-a"}, Headers{Kind: System.Kind()}); err != nil {
			t.Fatalf("unexpected emit error: %v", err)
		}
	}
	...
	if _, err := runtime.EmitWithHeaders(ctx, topic.Name, runtimeTestPayload{Message: "loop-a"}, Headers{SkipUniqueKey: true, Kind: System.Kind()}); err != nil {
		t.Fatalf("unexpected emit error: %v", err)
	}
	...
	if _, err := runtime.EmitWithHeaders(ctx, topic.Name, runtimeTestPayload{Message: "loop-b"}, Headers{Kind: System.Kind()}); err != nil {
		t.Fatalf("unexpected emit error: %v", err)
	}
