...
func TestDispatchTimeTopicRenameFallback(t *testing.T) {
	...
	newTopic := Topic[runtimeTestPayload]{Name: TopicName(TopicPrefixMutation + "GalaRenameFallback"), Kind: JobKindMutation}
	...
	if err := legacy.Gala.dispatcher.Dispatch(ctx, Envelope{
		ID:              NewEventID(),
		Topic:           oldTopic,
