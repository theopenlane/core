...
// RecordStorageRestore records the outcome of restoring a file from a backup provider back to its
// source provider. On success it also records the number of bytes restored
func RecordStorageRestore(source, destination string, bytes int64, duration float64, err error) {
	status := "success"
	if err != nil {
		status = "failed"
	}

	StorageRestoreAttempts.WithLabelValues(source, destination, status).Inc()
	StorageRestoreDuration.WithLabelValues(source, destination, status).Observe(duration)

	if err == nil {
		StorageRestoreBytes.WithLabelValues(source, destination).Add(float64(bytes))
	}
}
