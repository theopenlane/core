...
var (
	...
	// StorageRestoreAttempts tracks restore attempts copying a replica back to its source provider
	StorageRestoreAttempts = prometheus.NewCounterVec(prometheus.CounterOpts{
		Name: "openlane_storage_restore_attempts_total",
		Help: "Total number of file restore attempts by backup provider, source provider, and status (success, failed)",
	}, []string{"source", "destination", "status"})

	// StorageRestoreBytes tracks total bytes successfully restored to a source provider
	StorageRestoreBytes = prometheus.NewCounterVec(prometheus.CounterOpts{
		Name: "openlane_storage_restore_bytes_total",
		Help: "Total bytes successfully restored from a backup provider to a source provider",
	}, []string{"source", "destination"})

	// StorageRestoreDuration records the duration of restore operations
	StorageRestoreDuration = prometheus.NewHistogramVec(prometheus.HistogramOpts{
		Name:    "openlane_storage_restore_duration_seconds",
		Help:    "Duration of file restore operations by backup provider, source provider, and status",
		Buckets: prometheus.DefBuckets,
	}, []string{"source", "destination", "status"})
	...
	APIMetrics = []prometheus.Collector{
		...
		StorageRestoreAttempts,
		StorageRestoreBytes,
		StorageRestoreDuration,
