...
func NewConfigOpts(opts ...ConfigOpts) *Options {
	options := &Options{}
	...
	if options.Domain == nil {
		panic("options.Domain cannot be nil")
	}

	if options.DomainsFile == "" && len(options.Domain) == 0 {
		panic("options.DomainsFile cannot be empty")
	}
...
func WithDomain(domain string) ConfigOpts {
	return func(o *Options) {
		if err := o.Domain.Set(domain); err != nil {
			panic(err)
		}
	}
}
...
func NewSubfinder(opts ...SubfinderOpts) *Subfinder {
	subfinder := &Subfinder{}
	...
	if subfinder.Options == nil {
		panic("subfinder.Options cannot be nil")
	}

	subrunner, err := runner.NewRunner(&runner.Options{
		Verbose:            subfinder.Options.Verbose,
		NoColor:            subfinder.Options.NoColor,
		JSON:               subfinder.Options.JSON,
		HostIP:             subfinder.Options.HostIP,
		Silent:             subfinder.Options.Silent,
		ListSources:        subfinder.Options.ListSources,
		RemoveWildcard:     subfinder.Options.RemoveWildcard,
		CaptureSources:     subfinder.Options.CaptureSources,
		Stdin:              subfinder.Options.Stdin,
		Version:            subfinder.Options.Version,
		OnlyRecursive:      subfinder.Options.OnlyRecursive,
		All:                subfinder.Options.All,
		Statistics:         subfinder.Options.Statistics,
		Threads:            subfinder.Options.Threads,
		Timeout:            subfinder.Options.Timeout,
		MaxEnumerationTime: subfinder.Options.MaxEnumerationTime,
		Domain:             subfinder.Options.Domain,
		DomainsFile:        subfinder.Options.DomainsFile,
		Output:             subfinder.Options.Output,
		OutputFile:         subfinder.Options.OutputFile,
		OutputDirectory:    subfinder.Options.OutputDirectory,
		Sources:            subfinder.Options.Sources,
		ExcludeSources:     subfinder.Options.ExcludeSources,
		Resolvers:          subfinder.Options.Resolvers,
		ResolverList:       subfinder.Options.ResolverList,
		Config:             subfinder.Options.Config,
		ProviderConfig:     subfinder.Options.ProviderConfig,
		Proxy:              subfinder.Options.Proxy,
		RateLimit:          subfinder.Options.RateLimit,
		RateLimits:         subfinder.Options.RateLimits,
		ExcludeIps:         subfinder.Options.ExcludeIps,
		Match:              subfinder.Options.Match,
		Filter:             subfinder.Options.Filter,
		DisableUpdateCheck: subfinder.Options.DisableUpdateCheck,
	})
	if err != nil {
		panic(err)
	}
	...
	subfinder.Resolver = resolver
	subfinder.Runner = subrunner

	if subfinder.Resolver == nil {
		subfinder.Resolver = resolve.New()
	}

	return subfinder
}

func NewWithDefaults(domain string, opts ...ConfigOpts) *Options {
	options := &Options{
		...
		MaxEnumerationTime: 60, // nolint:mnd
		OutputFile:         "domainsoutput.txt",
	}
...
func NewResolverPool(workers int, removeWildcard bool) *resolve.ResolutionPool {
	return resolve.New().NewResolutionPool(workers, removeWildcard)
}

func NewSubfinderWithConfig(domain string, config *Options) *Subfinder {
	subfinder := NewSubfinder(WithConfig(config))
	...
	if config.Domain == nil {
		panic("config.Domain cannot be nil")
	}

	if config.DomainsFile == "" && len(config.Domain) == 0 {
		panic("config.DomainsFile cannot be empty")
	}

	if domain != "" {
		config.Domain = goflags.StringSlice{domain}
	}
