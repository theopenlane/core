...
func TestConfigureProviderRulesDevMode(t *testing.T) {
	...
	require.Equal(t, objstore.DefaultDevStorageBucket, result.Config.Bucket)
	require.Equal(t, objstore.DefaultDevStorageBucket, result.Config.BasePath)
...
func TestKnownProviderRule(t *testing.T) {
	ctx := contextx.With(context.Background(), objstore.KnownProviderHint(storage.DiskProvider))
	resolver := eddy.NewResolver[storage.Provider, storage.ProviderCredentials, *storage.ProviderOptions]()
...
func TestModuleRules(t *testing.T) {
	ctx := contextx.With(context.Background(), objstore.ModuleHint(models.CatalogTrustCenterModule))
	resolver := eddy.NewResolver[storage.Provider, storage.ProviderCredentials, *storage.ProviderOptions]()
