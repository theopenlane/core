...
func writeModuleConstants(path string, c models.Catalog) error {
	...
	f.Comment("modules and addons available in the catalog")
	...
	// lets generators that parse the constant out of the source resolve it back to its value
	f.Comment("OrgModuleByConstName maps the name of each module constant to the module it holds")
	f.Var().Id("OrgModuleByConstName").Op("=").Map(jen.String()).Id("OrgModule").Values(jen.DictFunc(func(d jen.Dict) {
		for _, k := range modKeys {
			name := "Catalog" + strcase.UpperCamelCase(k)
	...
	f.Comment("TrialModules contains the modules and addons included with a trial")
