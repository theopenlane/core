...
ALTER TABLE "subprocessors" DROP CONSTRAINT "subprocessors_files_logo_file", DROP COLUMN "logo_file_id", ADD COLUMN "logo_local_file_id" character varying NULL, ADD CONSTRAINT "subprocessors_files_logo_file" FOREIGN KEY ("logo_local_file_id") REFERENCES "files" ("id") ON UPDATE NO ACTION ON DELETE SET NULL;
-- reverse: modify "groups" table
ALTER TABLE "groups" DROP CONSTRAINT "groups_organizations_subprocessor_creators", DROP COLUMN "organization_subprocessor_creators";
-- reverse: modify "subprocessor_history" table
ALTER TABLE "subprocessor_history" DROP COLUMN "logo_file_id", ADD COLUMN "logo_local_file_id" character varying NULL;
