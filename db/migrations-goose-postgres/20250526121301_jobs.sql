...
CREATE TABLE "job_results" ("id" character varying NOT NULL, "created_at" timestamptz NULL, "updated_at" timestamptz NULL, "created_by" character varying NULL, "updated_by" character varying NULL, "deleted_at" timestamptz NULL, "deleted_by" character varying NULL, "status" character varying NOT NULL, "exit_code" bigint NOT NULL, "finished_at" timestamptz NOT NULL, "started_at" timestamptz NOT NULL, "scheduled_job_id" character varying NOT NULL, "file_id" character varying NOT NULL, "owner_id" character varying NULL, PRIMARY KEY ("id"), CONSTRAINT "job_results_control_scheduled_jobs_scheduled_job" FOREIGN KEY ("scheduled_job_id") REFERENCES "control_scheduled_jobs" ("id") ON UPDATE NO ACTION ON DELETE NO ACTION, CONSTRAINT "job_results_files_file" FOREIGN KEY ("file_id") REFERENCES "files" ("id") ON UPDATE NO ACTION ON DELETE NO ACTION, CONSTRAINT "job_results_organizations_scheduled_job_results" FOREIGN KEY ("owner_id") REFERENCES "organizations" ("id") ON UPDATE NO ACTION ON DELETE SET NULL);
