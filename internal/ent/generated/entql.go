...
var schemaGraph = func() *sqlgraph.Schema {
	...
	graph.Nodes[11] = &sqlgraph.Node{
		...
		Fields: map[string]*sqlgraph.FieldSpec{
			...
			controlscheduledjob.FieldControlID:      {Type: field.TypeString, Column: controlscheduledjob.FieldControlID},
	...
	graph.Nodes[63] = &sqlgraph.Node{
		...
		Fields: map[string]*sqlgraph.FieldSpec{
			...
			scheduledjob.FieldIsMarketplace: {Type: field.TypeBool, Column: scheduledjob.FieldIsMarketplace},
		},
	}
...
// WhereHasControlScheduledJobs applies a predicate to check if query has an edge control_scheduled_jobs.
func (f *ControlFilter) WhereHasControlScheduledJobs() {
	f.Where(entql.HasEdge("control_scheduled_jobs"))
}

// WhereHasControlScheduledJobsWith applies a predicate to check if query has an edge control_scheduled_jobs with a given conditions (other predicates).
func (f *ControlFilter) WhereHasControlScheduledJobsWith(preds ...predicate.ControlScheduledJob) {
	f.Where(entql.HasEdgeWith("control_scheduled_jobs", sqlgraph.WrapFunc(func(s *sql.Selector) {
		for _, p := range preds {
			p(s)
		}
	})))
...
func (f *ControlScheduledJobFilter) WhereControlID(p entql.StringP) {
	f.Where(p.Field(controlscheduledjob.FieldControlID))
}
...
func (f *ControlScheduledJobFilter) WhereHasControl() {
...
func (f *ControlScheduledJobFilter) WhereHasControlWith(preds ...predicate.Control) {
...
// WhereIsMarketplace applies the entql bool predicate on the is_marketplace field.
func (f *ScheduledJobFilter) WhereIsMarketplace(p entql.BoolP) {
	f.Where(p.Field(scheduledjob.FieldIsMarketplace))
}
