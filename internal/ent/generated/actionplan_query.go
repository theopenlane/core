...
func (_q *ActionPlanQuery) loadActionPlanKind(ctx context.Context, query *CustomTypeEnumQuery, nodes []*ActionPlan, init func(*ActionPlan), assign func(*ActionPlan, *CustomTypeEnum)) error {
	...
	nodeids := make(map[string][]*ActionPlan)
	for i := range nodes {
		if nodes[i].action_plan_action_plan_kind == nil {
			continue
		}
		fk := *nodes[i].action_plan_action_plan_kind
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "action_plan_action_plan_kind" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
