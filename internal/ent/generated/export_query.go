...
func (eq *ExportQuery) QueryFile() *FileQuery {
	query := (&FileClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (eq *ExportQuery) WithFile(opts ...func(*FileQuery)) *ExportQuery {
	query := (&FileClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *ExportQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Export, error) {
	...
	if query := eq.withFile; query != nil {
		if err := eq.loadFile(ctx, query, nodes, nil,
			func(n *Export, e *File) { n.Edges.File = e }); err != nil {
			return nil, err
		}
	}
...
func (eq *ExportQuery) loadFiles(ctx context.Context, query *FileQuery, nodes []*Export, init func(*Export), assign func(*Export, *File)) error {
	...
	nids := make(map[string]map[*Export]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(export.FilesTable)
		joinT.Schema(eq.schemaConfig.ExportFiles)
		s.Join(joinT).On(s.C(file.FieldID), joinT.C(export.FilesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(export.FilesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(export.FilesPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Export]struct{}{byID[outValue]: {}}
...
func (eq *ExportQuery) loadFile(ctx context.Context, query *FileQuery, nodes []*Export, init func(*Export), assign func(*Export, *File)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Export)
	for i := range nodes {
		fk := nodes[i].FileID
...
func (eq *ExportQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := eq.ctx.Fields; len(fields) > 0 {
		...
		if eq.withFile != nil {
			_spec.Node.AddColumnOnce(export.FieldFileID)
		}
	}
