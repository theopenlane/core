...
func (_q *RiskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Risk, error) {
	...
	if _q.withRiskKind != nil || _q.withRiskCategory != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *RiskQuery) loadRiskKind(ctx context.Context, query *CustomTypeEnumQuery, nodes []*Risk, init func(*Risk), assign func(*Risk, *CustomTypeEnum)) error {
	...
	nodeids := make(map[string][]*Risk)
	for i := range nodes {
		if nodes[i].risk_risk_kind == nil {
			continue
		}
		fk := *nodes[i].risk_risk_kind
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "risk_risk_kind" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RiskQuery) loadRiskCategory(ctx context.Context, query *CustomTypeEnumQuery, nodes []*Risk, init func(*Risk), assign func(*Risk, *CustomTypeEnum)) error {
	...
	nodeids := make(map[string][]*Risk)
	for i := range nodes {
		if nodes[i].risk_risk_category == nil {
			continue
		}
		fk := *nodes[i].risk_risk_category
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "risk_risk_category" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
