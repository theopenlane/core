...
// The ExportHistoryFunc type is an adapter to allow the use of ordinary function as a Querier.
type ExportHistoryFunc func(context.Context, *generated.ExportHistoryQuery) (generated.Value, error)

// Query calls f(ctx, q).
func (f ExportHistoryFunc) Query(ctx context.Context, q generated.Query) (generated.Value, error) {
	if q, ok := q.(*generated.ExportHistoryQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *generated.ExportHistoryQuery", q)
}

// The TraverseExportHistory type is an adapter to allow the use of ordinary function as Traverser.
type TraverseExportHistory func(context.Context, *generated.ExportHistoryQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseExportHistory) Intercept(next generated.Querier) generated.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseExportHistory) Traverse(ctx context.Context, q generated.Query) error {
	if q, ok := q.(*generated.ExportHistoryQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *generated.ExportHistoryQuery", q)
}
...
func NewQuery(q generated.Query) (Query, error) {
	...
	case *generated.ExportHistoryQuery:
		return &query[*generated.ExportHistoryQuery, predicate.ExportHistory, exporthistory.OrderOption]{typ: generated.TypeExportHistory, tq: q}, nil
