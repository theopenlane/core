...
func (opq *OrgProductQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "OrgProduct").Str("operation", "count_ids").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, opq.ctx, ent.OpQueryIDs)

	ids, err := opq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "OrgProduct").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")

		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "OrgProduct").Str("operation", "count_ids").Int("count", len(ids)).Msg("CountIDs: completed")
