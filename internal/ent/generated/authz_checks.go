...
func (q *TrustCenterControlQuery) CheckAccess(ctx context.Context) error {
	gCtx := graphql.GetFieldContext(ctx)
	...
	whereArg := gCtx.Args["where"]
	if whereArg != nil {
		where, ok := whereArg.(*TrustCenterControlWhereInput)
		if ok && where != nil && where.TrustCenterID != nil {
			objectID = *where.TrustCenterID
		}
	}
...
func (m *TrustCenterControlMutation) CheckAccessForEdit(ctx context.Context) error {
	var objectID string
	...
	// check if the input is a CreateTrustCenterControlInput
	input, ok := gInput.(CreateTrustCenterControlInput)
	if ok {
		objectID = *input.TrustCenterID
	...
	if objectID == "" {
		...
		if ok {
			// allow this query to run
			reqCtx := privacy.DecisionContext(ctx, privacy.Allow)
			ob, err := m.Client().TrustCenterControl.Query().Where(trustcentercontrol.ID(id)).Only(reqCtx)
			if err != nil {
				return privacy.Skipf("nil request, skipping auth check")
			}
			objectID = ob.TrustCenterID
...
func (m *TrustCenterControlMutation) CheckAccessForDelete(ctx context.Context) error {
	gCtx := graphql.GetFieldContext(ctx)
	if gCtx == nil {
		// Skip to the next privacy rule (equivalent to return nil)
		// if this is not a graphql request
...
func (q *TrustCenterControlHistoryQuery) CheckAccess(ctx context.Context) error {
	gCtx := graphql.GetFieldContext(ctx)
	...
	whereArg := gCtx.Args["where"]
	if whereArg != nil {
		where, ok := whereArg.(*TrustCenterControlHistoryWhereInput)
		if ok && where != nil && where.TrustCenterID != nil {
			objectID = *where.TrustCenterID
		}
	}
