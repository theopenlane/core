...
func (*Note) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case note.FieldID, note.FieldCreatedBy, note.FieldUpdatedBy, note.FieldUpdatedByImpersonator, note.FieldDeletedBy, note.FieldDisplayID, note.FieldOwnerID, note.FieldTitle, note.FieldText, note.FieldNoteRef, note.FieldDiscussionID, note.FieldTrustCenterID:
...
func (_m *Note) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case note.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
