...
type ScheduledJobQuery struct {
	...
	withScheduledJobSetting *ScheduledJobSettingQuery
...
// QueryScheduledJobSetting chains the current query on the "scheduled_job_setting" edge.
func (sjq *ScheduledJobQuery) QueryScheduledJobSetting() *ScheduledJobSettingQuery {
	query := (&ScheduledJobSettingClient{config: sjq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sjq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(scheduledjob.Table, scheduledjob.FieldID, selector),
			sqlgraph.To(scheduledjobsetting.Table, scheduledjobsetting.FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, scheduledjob.ScheduledJobSettingTable, scheduledjob.ScheduledJobSettingColumn),
		)
		schemaConfig := sjq.schemaConfig
...
func (sjq *ScheduledJobQuery) Clone() *ScheduledJobQuery {
	...
	return &ScheduledJobQuery{
		...
		withScheduledJobSetting: sjq.withScheduledJobSetting.Clone(),
...
// WithScheduledJobSetting tells the query-builder to eager-load the nodes that are connected to
// the "scheduled_job_setting" edge. The optional arguments are used to configure the query builder of the edge.
func (sjq *ScheduledJobQuery) WithScheduledJobSetting(opts ...func(*ScheduledJobSettingQuery)) *ScheduledJobQuery {
	query := (&ScheduledJobSettingClient{config: sjq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sjq.withScheduledJobSetting = query
	return sjq
}
...
func (sjq *ScheduledJobQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ScheduledJob, error) {
	var (
		...
		loadedTypes = [2]bool{
			sjq.withOwner != nil,
			sjq.withScheduledJobSetting != nil,
		}
	)
	...
	if query := sjq.withScheduledJobSetting; query != nil {
		if err := sjq.loadScheduledJobSetting(ctx, query, nodes, nil,
			func(n *ScheduledJob, e *ScheduledJobSetting) { n.Edges.ScheduledJobSetting = e }); err != nil {
			return nil, err
		}
	}
...
func (sjq *ScheduledJobQuery) loadScheduledJobSetting(ctx context.Context, query *ScheduledJobSettingQuery, nodes []*ScheduledJob, init func(*ScheduledJob), assign func(*ScheduledJob, *ScheduledJobSetting)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(scheduledjobsetting.FieldScheduledJobID)
	}
	query.Where(predicate.ScheduledJobSetting(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(scheduledjob.ScheduledJobSettingColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "scheduled_job_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
