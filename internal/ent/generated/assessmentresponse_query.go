...
func (arq *AssessmentResponseQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "AssessmentResponse").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, arq.ctx, ent.OpQueryIDs)

	ids, err := arq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "AssessmentResponse").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "AssessmentResponse").Int("count", len(ids)).Msg("CountIDs: completed")
