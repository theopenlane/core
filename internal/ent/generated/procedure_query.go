...
func (_q *ProcedureQuery) loadProcedureKind(ctx context.Context, query *CustomTypeEnumQuery, nodes []*Procedure, init func(*Procedure), assign func(*Procedure, *CustomTypeEnum)) error {
	...
	nodeids := make(map[string][]*Procedure)
	for i := range nodes {
		if nodes[i].procedure_procedure_kind == nil {
			continue
		}
		fk := *nodes[i].procedure_procedure_kind
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "procedure_procedure_kind" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
