...
func (wq *WebauthnQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "Webauthn").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, wq.ctx, ent.OpQueryIDs)

	ids, err := wq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "Webauthn").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "Webauthn").Int("count", len(ids)).Msg("CountIDs: completed")
