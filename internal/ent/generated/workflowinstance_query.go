...
func (wiq *WorkflowInstanceQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "WorkflowInstance").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, wiq.ctx, ent.OpQueryIDs)

	ids, err := wiq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "WorkflowInstance").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "WorkflowInstance").Int("count", len(ids)).Msg("CountIDs: completed")
