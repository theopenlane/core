...
func (_q *NotificationQuery) QueryOwner() *OrganizationQuery {
...
func (_q *NotificationQuery) WithOwner(opts ...func(*OrganizationQuery)) *NotificationQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
...
func (_q *NotificationQuery) prepareQuery(ctx context.Context) error {
	...
	if notification.Policy == nil {
		return errors.New("generated: uninitialized notification.Policy (forgotten import generated/runtime?)")
	}
	if err := notification.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	_spec.Node.Schema = _q.schemaConfig.Notification
...
func (_q *NotificationQuery) loadOwner(ctx context.Context, query *OrganizationQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Organization)) error {
...
func (_q *NotificationQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.Notification)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
...
func (nq *NotificationQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "Notification").Str("operation", "count_ids").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, nq.ctx, ent.OpQueryIDs)

	ids, err := nq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "Notification").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")

		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "Notification").Str("operation", "count_ids").Int("count", len(ids)).Msg("CountIDs: completed")
