...
func (*OrgSubscriptionHistory) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case orgsubscriptionhistory.FieldTags, orgsubscriptionhistory.FieldFeatures:
			values[i] = new([]byte)
...
func (osh *OrgSubscriptionHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case orgsubscriptionhistory.FieldCreatedByID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field created_by_id", values[i])
			} else if value.Valid {
				osh.CreatedByID = value.String
			}
		case orgsubscriptionhistory.FieldUpdatedByID:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case orgsubscriptionhistory.FieldDeletedByID:
			if value, ok := values[i].(*sql.NullString); !ok {
