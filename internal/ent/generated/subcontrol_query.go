...
func (sq *SubcontrolQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "Subcontrol").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, sq.ctx, ent.OpQueryIDs)

	ids, err := sq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "Subcontrol").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "Subcontrol").Int("count", len(ids)).Msg("CountIDs: completed")
