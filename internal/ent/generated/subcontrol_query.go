...
func (_q *SubcontrolQuery) loadSubcontrolKind(ctx context.Context, query *CustomTypeEnumQuery, nodes []*Subcontrol, init func(*Subcontrol), assign func(*Subcontrol, *CustomTypeEnum)) error {
	...
	nodeids := make(map[string][]*Subcontrol)
	for i := range nodes {
		if nodes[i].subcontrol_subcontrol_kind == nil {
			continue
		}
		fk := *nodes[i].subcontrol_subcontrol_kind
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "subcontrol_subcontrol_kind" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
