...
func (_m *IntegrationRecommendation) Owner(ctx context.Context) (*Organization, error) {
	result, err := _m.Edges.OwnerOrErr()
	if IsNotLoaded(err) {
		result, err = _m.QueryOwner().Only(ctx)
	}
...
func (_m *Organization) IntegrationRecommendations(
	ctx context.Context, after *Cursor, first *int, before *Cursor, last *int, orderBy []*IntegrationRecommendationOrder, where *IntegrationRecommendationWhereInput,
) (*IntegrationRecommendationConnection, error) {
	opts := []IntegrationRecommendationPaginateOption{
		WithIntegrationRecommendationOrder(orderBy),
		WithIntegrationRecommendationFilter(where.Filter),
	}
	alias := graphql.GetFieldContext(ctx).Field.Alias
	totalCount, hasTotalCount := _m.Edges.totalCount[87][alias]
	if nodes, err := _m.NamedIntegrationRecommendations(alias); err == nil || hasTotalCount {
		pager, err := newIntegrationRecommendationPager(opts, last != nil)
		if err != nil {
			return nil, err
		}
		conn := &IntegrationRecommendationConnection{Edges: []*IntegrationRecommendationEdge{}, TotalCount: totalCount}
		conn.build(nodes, pager, after, first, before, last)
		return conn, nil
	}
	return _m.QueryIntegrationRecommendations().Paginate(ctx, after, first, before, last, opts...)
}
