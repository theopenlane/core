...
func (oq *OrganizationQuery) loadFiles(ctx context.Context, query *FileQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *File)) error {
	...
	for _, n := range neighbors {
		fk := n.organization_files
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_files" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_files" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
