...
func (_q *OrganizationQuery) QueryTemplateResponders() *TemplateResponderQuery {
...
func (_q *OrganizationQuery) WithTemplateResponders(opts ...func(*TemplateResponderQuery)) *OrganizationQuery {
	query := (&TemplateResponderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withTemplateResponders; query != nil {
		if err := _q.loadTemplateResponders(ctx, query, nodes,
			func(n *Organization) { n.Edges.TemplateResponders = []*TemplateResponder{} },
			func(n *Organization, e *TemplateResponder) {
	...
	for name, query := range _q.withNamedTemplateResponders {
		if err := _q.loadTemplateResponders(ctx, query, nodes,
			func(n *Organization) { n.appendNamedTemplateResponders(name) },
			func(n *Organization, e *TemplateResponder) { n.appendNamedTemplateResponders(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrganizationQuery) loadTemplateResponders(ctx context.Context, query *TemplateResponderQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *TemplateResponder)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(templateresponder.FieldOwnerID)
	}
	query.Where(predicate.TemplateResponder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.TemplateRespondersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithNamedTemplateResponders(name string, opts ...func(*TemplateResponderQuery)) *OrganizationQuery {
	query := (&TemplateResponderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
