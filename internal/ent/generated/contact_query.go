...
func (_q *ContactQuery) QueryIntegrationRuns() *IntegrationRunQuery {
...
func (_q *ContactQuery) QueryAudienceMembers() *AudienceMemberQuery {
...
func (_q *ContactQuery) WithIntegrationRuns(opts ...func(*IntegrationRunQuery)) *ContactQuery {
	query := (&IntegrationRunClient{config: _q.config}).Query()
...
func (_q *ContactQuery) WithAudienceMembers(opts ...func(*AudienceMemberQuery)) *ContactQuery {
	query := (&AudienceMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ContactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Contact, error) {
	...
	if query := _q.withIntegrationRuns; query != nil {
		if err := _q.loadIntegrationRuns(ctx, query, nodes,
			func(n *Contact) { n.Edges.IntegrationRuns = []*IntegrationRun{} },
			func(n *Contact, e *IntegrationRun) { n.Edges.IntegrationRuns = append(n.Edges.IntegrationRuns, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAudienceMembers; query != nil {
		if err := _q.loadAudienceMembers(ctx, query, nodes,
			func(n *Contact) { n.Edges.AudienceMembers = []*AudienceMember{} },
			func(n *Contact, e *AudienceMember) { n.Edges.AudienceMembers = append(n.Edges.AudienceMembers, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedIntegrationRuns {
		if err := _q.loadIntegrationRuns(ctx, query, nodes,
			func(n *Contact) { n.appendNamedIntegrationRuns(name) },
			func(n *Contact, e *IntegrationRun) { n.appendNamedIntegrationRuns(name, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedAudienceMembers {
		if err := _q.loadAudienceMembers(ctx, query, nodes,
			func(n *Contact) { n.appendNamedAudienceMembers(name) },
			func(n *Contact, e *AudienceMember) { n.appendNamedAudienceMembers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ContactQuery) loadIntegrationRuns(ctx context.Context, query *IntegrationRunQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *IntegrationRun)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(contact.IntegrationRunsTable)
		s.Join(joinT).On(s.C(integrationrun.FieldID), joinT.C(contact.IntegrationRunsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(contact.IntegrationRunsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(contact.IntegrationRunsPrimaryKey[0]))
...
func (_q *ContactQuery) loadAudienceMembers(ctx context.Context, query *AudienceMemberQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *AudienceMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Contact)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(audiencemember.FieldContactID)
	}
	query.Where(predicate.AudienceMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(contact.AudienceMembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ContactQuery) WithNamedIntegrationRuns(name string, opts ...func(*IntegrationRunQuery)) *ContactQuery {
	query := (&IntegrationRunClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *ContactQuery) WithNamedAudienceMembers(name string, opts ...func(*AudienceMemberQuery)) *ContactQuery {
	query := (&AudienceMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ContactQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "Contact").Str("operation", "count_ids").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, cq.ctx, ent.OpQueryIDs)

	ids, err := cq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "Contact").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")

		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "Contact").Str("operation", "count_ids").Int("count", len(ids)).Msg("CountIDs: completed")
