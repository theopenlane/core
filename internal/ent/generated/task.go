...
func (*Task) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case task.FieldID, task.FieldCreatedBy, task.FieldUpdatedBy, task.FieldDeletedBy, task.FieldDisplayID, task.FieldOwnerID, task.FieldTitle, task.FieldDetails, task.FieldStatus, task.FieldCategory, task.FieldAssigneeID, task.FieldAssignerID, task.FieldIdempotencyKey:
			values[i] = new(sql.NullString)
		case task.FieldCreatedAt, task.FieldUpdatedAt, task.FieldDeletedAt:
			values[i] = new(sql.NullTime)
		case task.ForeignKeys[0]: // custom_type_enum_tasks
			values[i] = new(sql.NullString)
		case task.ForeignKeys[1]: // task_task_kind
			values[i] = new(sql.NullString)
		default:
...
func (_m *Task) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case task.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field task_task_kind", values[i])
			} else if value.Valid {
				_m.task_task_kind = new(string)
				*_m.task_task_kind = value.String
			}
		default:
