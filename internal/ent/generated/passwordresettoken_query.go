...
func (prtq *PasswordResetTokenQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "PasswordResetToken").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, prtq.ctx, ent.OpQueryIDs)

	ids, err := prtq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "PasswordResetToken").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "PasswordResetToken").Int("count", len(ids)).Msg("CountIDs: completed")
