...
func (sjc *ScheduledJobCreate) SetScheduledJobSettingID(id string) *ScheduledJobCreate {
	sjc.mutation.SetScheduledJobSettingID(id)
	return sjc
}

// SetScheduledJobSetting sets the "scheduled_job_setting" edge to the ScheduledJobSetting entity.
func (sjc *ScheduledJobCreate) SetScheduledJobSetting(s *ScheduledJobSetting) *ScheduledJobCreate {
	return sjc.SetScheduledJobSettingID(s.ID)
}
...
func (sjc *ScheduledJobCreate) createSpec() (*ScheduledJob, *sqlgraph.CreateSpec) {
	...
	if nodes := sjc.mutation.ScheduledJobSettingIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2O,
			Inverse: false,
			...
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(scheduledjobsetting.FieldID, field.TypeString),
			},
		}
		edge.Schema = sjc.schemaConfig.ScheduledJobSetting
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges = append(_spec.Edges, edge)
