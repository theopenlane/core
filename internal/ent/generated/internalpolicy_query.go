...
func (_q *InternalPolicyQuery) loadInternalPolicyKind(ctx context.Context, query *CustomTypeEnumQuery, nodes []*InternalPolicy, init func(*InternalPolicy), assign func(*InternalPolicy, *CustomTypeEnum)) error {
	...
	nodeids := make(map[string][]*InternalPolicy)
	for i := range nodes {
		if nodes[i].internal_policy_internal_policy_kind == nil {
			continue
		}
		fk := *nodes[i].internal_policy_internal_policy_kind
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "internal_policy_internal_policy_kind" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
