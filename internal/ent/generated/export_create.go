...
func (ec *ExportCreate) SetItemID(s string) *ExportCreate {
	ec.mutation.SetItemID(s)
	return ec
}
...
func (ec *ExportCreate) SetFileID(s string) *ExportCreate {
	ec.mutation.SetFileID(s)
	return ec
}

// SetNillableFileID sets the "file_id" field if the given value is not nil.
func (ec *ExportCreate) SetNillableFileID(s *string) *ExportCreate {
	if s != nil {
		ec.SetFileID(*s)
	}
	return ec
...
func (ec *ExportCreate) SetFile(f *File) *ExportCreate {
...
func (ec *ExportCreate) check() error {
	...
	if _, ok := ec.mutation.ItemID(); !ok {
		return &ValidationError{Name: "item_id", err: errors.New(`generated: missing required field "Export.item_id"`)}
	}
	if v, ok := ec.mutation.ItemID(); ok {
		if err := export.ItemIDValidator(v); err != nil {
			return &ValidationError{Name: "item_id", err: fmt.Errorf(`generated: validator failed for field "Export.item_id": %w`, err)}
		}
	}
	if _, ok := ec.mutation.Status(); !ok {
...
func (ec *ExportCreate) createSpec() (*Export, *sqlgraph.CreateSpec) {
	...
	if value, ok := ec.mutation.ItemID(); ok {
		_spec.SetField(export.FieldItemID, field.TypeString, value)
		_node.ItemID = value
	}
	...
	if nodes := ec.mutation.FilesIDs(); len(nodes) > 0 {
		...
		edge.Schema = ec.schemaConfig.ExportFiles
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges = append(_spec.Edges, edge)
