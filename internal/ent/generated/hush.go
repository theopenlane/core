...
func (*Hush) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case hush.FieldID, hush.FieldCreatedBy, hush.FieldUpdatedBy, hush.FieldUpdatedByImpersonator, hush.FieldDeletedBy, hush.FieldOwnerID, hush.FieldInternalNotes, hush.FieldSystemInternalID, hush.FieldName, hush.FieldDescription, hush.FieldKind, hush.FieldSecretName, hush.FieldSecretValue:
...
func (_m *Hush) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case hush.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
