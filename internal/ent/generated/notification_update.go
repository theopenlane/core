...
func (_u *NotificationUpdate) check() error {
	...
	if v, ok := _u.mutation.Channels(); ok {
		if err := notification.ChannelsValidator(v); err != nil {
			return &ValidationError{Name: "channels", err: fmt.Errorf(`generated: validator failed for field "Notification.channels": %w`, err)}
		}
	}
	return nil
}
