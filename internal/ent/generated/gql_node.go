...
var exporthistoryImplementors = []string{"ExportHistory", "Node"}

// IsNode implements the Node interface check for GQLGen.
func (*ExportHistory) IsNode() {}
...
func (c *Client) noder(ctx context.Context, table string, id string) (Noder, error) {
	...
	case export.Table:
		...
		return query.Only(ctx)
	case exporthistory.Table:
		query := c.ExportHistory.Query().
			Where(exporthistory.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, exporthistoryImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []string) ([]Noder, error) {
	...
	case exporthistory.Table:
		query := c.ExportHistory.Query().
			Where(exporthistory.IDIn(ids...))
		query, err := query.CollectFields(ctx, exporthistoryImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
