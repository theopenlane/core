...
func (_q *NoteQuery) QueryReview() *ReviewQuery {
...
func (_q *NoteQuery) QueryDiscussion() *DiscussionQuery {
...
func (_q *NoteQuery) WithReview(opts ...func(*ReviewQuery)) *NoteQuery {
	query := (&ReviewClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NoteQuery) WithDiscussion(opts ...func(*DiscussionQuery)) *NoteQuery {
	query := (&DiscussionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NoteQuery) loadReview(ctx context.Context, query *ReviewQuery, nodes []*Note, init func(*Note), assign func(*Note, *Review)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Note)
	for i := range nodes {
...
func (_q *NoteQuery) loadDiscussion(ctx context.Context, query *DiscussionQuery, nodes []*Note, init func(*Note), assign func(*Note, *Discussion)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Note)
	for i := range nodes {
		fk := nodes[i].DiscussionID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(discussion.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
...
func (nq *NoteQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "Note").Str("operation", "count_ids").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, nq.ctx, ent.OpQueryIDs)

	ids, err := nq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "Note").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")

		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "Note").Str("operation", "count_ids").Int("count", len(ids)).Msg("CountIDs: completed")
