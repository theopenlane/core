...
func (evtq *EmailVerificationTokenQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "EmailVerificationToken").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, evtq.ctx, ent.OpQueryIDs)

	ids, err := evtq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "EmailVerificationToken").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "EmailVerificationToken").Int("count", len(ids)).Msg("CountIDs: completed")
