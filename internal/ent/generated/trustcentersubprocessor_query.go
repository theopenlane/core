...
func (tcsq *TrustCenterSubprocessorQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "TrustCenterSubprocessor").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, tcsq.ctx, ent.OpQueryIDs)

	ids, err := tcsq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "TrustCenterSubprocessor").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "TrustCenterSubprocessor").Int("count", len(ids)).Msg("CountIDs: completed")
