...
func (_q *OrganizationSettingHistoryQuery) prepareQuery(ctx context.Context) error {
	...
	if organizationsettinghistory.Policy == nil {
		return errors.New("historygenerated: uninitialized organizationsettinghistory.Policy (forgotten import historygenerated/runtime?)")
	}
...
func (oshq *OrganizationSettingHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	...
	if err != nil {
		logx.ErrorEvent(ctx, err).Str("query_type", "OrganizationSettingHistory").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")
