...
func (_m *TaskHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case taskhistory.FieldWorkflowEligibleMarker:
...
func (_m *TaskHistory) Unwrap() *TaskHistory {
	...
	if !ok {
		panic("historygenerated: TaskHistory is not a transactional entity")
	}
