...
func (tehq *TrustcenterEntityHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "TrustcenterEntityHistory").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, tehq.ctx, ent.OpQueryIDs)

	ids, err := tehq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "TrustcenterEntityHistory").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "TrustcenterEntityHistory").Int("count", len(ids)).Msg("CountIDs: completed")
