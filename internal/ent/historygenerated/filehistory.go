...
func (_m *FileHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case filehistory.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
...
func (_m *FileHistory) Unwrap() *FileHistory {
	...
	if !ok {
		panic("historygenerated: FileHistory is not a transactional entity")
	}
