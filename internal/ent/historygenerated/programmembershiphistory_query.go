...
func (_q *ProgramMembershipHistoryQuery) prepareQuery(ctx context.Context) error {
	...
	if programmembershiphistory.Policy == nil {
		return errors.New("historygenerated: uninitialized programmembershiphistory.Policy (forgotten import historygenerated/runtime?)")
	}
...
func (pmhq *ProgramMembershipHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "ProgramMembershipHistory").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, pmhq.ctx, ent.OpQueryIDs)

	ids, err := pmhq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "ProgramMembershipHistory").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "ProgramMembershipHistory").Int("count", len(ids)).Msg("CountIDs: completed")
