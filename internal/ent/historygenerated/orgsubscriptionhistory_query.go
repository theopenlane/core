...
func (oshq *OrgSubscriptionHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "OrgSubscriptionHistory").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, oshq.ctx, ent.OpQueryIDs)

	ids, err := oshq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "OrgSubscriptionHistory").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "OrgSubscriptionHistory").Int("count", len(ids)).Msg("CountIDs: completed")
