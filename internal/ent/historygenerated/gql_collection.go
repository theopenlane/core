...
func (_q *EvidenceHistoryQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "nextReviewAt":
			if _, ok := fieldSeen[evidencehistory.FieldNextReviewAt]; !ok {
				selectedFields = append(selectedFields, evidencehistory.FieldNextReviewAt)
				fieldSeen[evidencehistory.FieldNextReviewAt] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
