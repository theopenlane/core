...
func (_m *DirectoryMembershipHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case directorymembershiphistory.FieldPlatformID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field platform_id", values[i])
			} else if value.Valid {
				_m.PlatformID = value.String
			}
		case directorymembershiphistory.FieldDirectoryInstanceID:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case directorymembershiphistory.FieldAddedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
		...
		case directorymembershiphistory.FieldRemovedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
