...
func (rhq *RiskHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "RiskHistory").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, rhq.ctx, ent.OpQueryIDs)

	ids, err := rhq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "RiskHistory").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "RiskHistory").Int("count", len(ids)).Msg("CountIDs: completed")
