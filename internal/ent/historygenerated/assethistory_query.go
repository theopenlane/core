...
func (_q *AssetHistoryQuery) prepareQuery(ctx context.Context) error {
	...
	if assethistory.Policy == nil {
		return errors.New("historygenerated: uninitialized assethistory.Policy (forgotten import historygenerated/runtime?)")
	}
...
func (ahq *AssetHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "AssetHistory").Str("operation", "count_ids").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, ahq.ctx, ent.OpQueryIDs)

	ids, err := ahq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "AssetHistory").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")

		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "AssetHistory").Str("operation", "count_ids").Int("count", len(ids)).Msg("CountIDs: completed")
