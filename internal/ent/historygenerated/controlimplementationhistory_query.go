...
func (_q *ControlImplementationHistoryQuery) prepareQuery(ctx context.Context) error {
	...
	if controlimplementationhistory.Policy == nil {
		return errors.New("historygenerated: uninitialized controlimplementationhistory.Policy (forgotten import historygenerated/runtime?)")
	}
...
func (cihq *ControlImplementationHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	...
	if err != nil {
		logx.ErrorEvent(ctx, err).Str("query_type", "ControlImplementationHistory").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")
