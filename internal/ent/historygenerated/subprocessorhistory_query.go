...
func (shq *SubprocessorHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "SubprocessorHistory").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, shq.ctx, ent.OpQueryIDs)

	ids, err := shq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "SubprocessorHistory").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "SubprocessorHistory").Int("count", len(ids)).Msg("CountIDs: completed")
