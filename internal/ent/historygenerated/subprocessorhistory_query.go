...
func (_q *SubprocessorHistoryQuery) prepareQuery(ctx context.Context) error {
	...
	if subprocessorhistory.Policy == nil {
		return errors.New("historygenerated: uninitialized subprocessorhistory.Policy (forgotten import historygenerated/runtime?)")
	}
...
func (shq *SubprocessorHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "SubprocessorHistory").Str("operation", "count_ids").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, shq.ctx, ent.OpQueryIDs)

	ids, err := shq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "SubprocessorHistory").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")

		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "SubprocessorHistory").Str("operation", "count_ids").Int("count", len(ids)).Msg("CountIDs: completed")
