...
func (gmhq *GroupMembershipHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "GroupMembershipHistory").Str("operation", "count_ids").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, gmhq.ctx, ent.OpQueryIDs)

	ids, err := gmhq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "GroupMembershipHistory").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")

		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "GroupMembershipHistory").Str("operation", "count_ids").Int("count", len(ids)).Msg("CountIDs: completed")
