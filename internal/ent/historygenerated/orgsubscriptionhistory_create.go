...
func (_c *OrgSubscriptionHistoryCreate) check() error {
	if _, ok := _c.mutation.HistoryTime(); !ok {
		return &ValidationError{Name: "history_time", err: errors.New(`historygenerated: missing required field "OrgSubscriptionHistory.history_time"`)}
	}
	if _, ok := _c.mutation.Operation(); !ok {
		return &ValidationError{Name: "operation", err: errors.New(`historygenerated: missing required field "OrgSubscriptionHistory.operation"`)}
	}
	if v, ok := _c.mutation.Operation(); ok {
		if err := orgsubscriptionhistory.OperationValidator(v); err != nil {
			return &ValidationError{Name: "operation", err: fmt.Errorf(`historygenerated: validator failed for field "OrgSubscriptionHistory.operation": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Active(); !ok {
		return &ValidationError{Name: "active", err: errors.New(`historygenerated: missing required field "OrgSubscriptionHistory.active"`)}
	}
	return nil
}
