...
func (_m *TrustCenterSubprocessorHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case trustcentersubprocessorhistory.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
...
func (_m *TrustCenterSubprocessorHistory) Unwrap() *TrustCenterSubprocessorHistory {
	...
	if !ok {
		panic("historygenerated: TrustCenterSubprocessorHistory is not a transactional entity")
	}
