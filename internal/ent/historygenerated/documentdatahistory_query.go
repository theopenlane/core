...
func (ddhq *DocumentDataHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	logx.FromContext(ctx).Debug().Str("query_type", "DocumentDataHistory").Msg("CountIDs: starting")

	ctx = setContextOp(ctx, ddhq.ctx, ent.OpQueryIDs)

	ids, err := ddhq.IDs(ctx)
	if err != nil {
		logx.FromContext(ctx).Error().Err(err).Str("query_type", "DocumentDataHistory").Msg("CountIDs: IDs() failed")
		return 0, err
	}

	logx.FromContext(ctx).Debug().Str("query_type", "DocumentDataHistory").Int("count", len(ids)).Msg("CountIDs: completed")
