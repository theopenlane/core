...
func (_q *NarrativeHistoryQuery) prepareQuery(ctx context.Context) error {
	...
	if narrativehistory.Policy == nil {
		return errors.New("historygenerated: uninitialized narrativehistory.Policy (forgotten import historygenerated/runtime?)")
	}
...
func (nhq *NarrativeHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	...
	ids, err := nhq.IDs(ctx)
	if err != nil {
		logx.ErrorEvent(ctx, err).Str("query_type", "NarrativeHistory").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")
