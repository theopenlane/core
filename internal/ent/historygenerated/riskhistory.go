...
func (*RiskHistory) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case riskhistory.FieldObservedAt, riskhistory.FieldMitigatedAt, riskhistory.FieldLastReviewedAt, riskhistory.FieldDueDate, riskhistory.FieldNextReviewDueAt:
...
func (_m *RiskHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case riskhistory.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case riskhistory.FieldIntegrationID:
			if value, ok := values[i].(*sql.NullString); !ok {
