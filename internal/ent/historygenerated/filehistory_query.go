...
func (_q *FileHistoryQuery) prepareQuery(ctx context.Context) error {
	...
	if filehistory.Policy == nil {
		return errors.New("historygenerated: uninitialized filehistory.Policy (forgotten import historygenerated/runtime?)")
	}
...
func (fhq *FileHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	...
	ids, err := fhq.IDs(ctx)
	if err != nil {
		logx.ErrorEvent(ctx, err).Str("query_type", "FileHistory").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")
