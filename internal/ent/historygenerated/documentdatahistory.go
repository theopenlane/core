...
func (_m *DocumentDataHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case documentdatahistory.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
...
func (_m *DocumentDataHistory) Unwrap() *DocumentDataHistory {
	...
	if !ok {
		panic("historygenerated: DocumentDataHistory is not a transactional entity")
	}
