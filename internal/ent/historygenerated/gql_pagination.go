...
func (_m *AssessmentResponseHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*AssessmentResponseHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
var (
	...
	// AssetHistoryOrderFieldObservedAt orders AssetHistory by observed_at.
	AssetHistoryOrderFieldObservedAt = &AssetHistoryOrderField{
		Value: func(_m *AssetHistory) (ent.Value, error) {
			// allow for nil values for fields
		...
		column: assethistory.FieldObservedAt,
		toTerm: func(opts ...sql.OrderTermOption) assethistory.OrderOption {
			opts = append(opts, sql.OrderNullsLast())
			return assethistory.ByObservedAt(opts...)
		},
		toCursor: func(_m *AssetHistory) Cursor {
			if _m.ObservedAt == nil {
				return Cursor{
...
func (f AssetHistoryOrderField) String() string {
	...
	case AssetHistoryOrderFieldPurchaseDate.column:
		str = "purchase_date"
	case AssetHistoryOrderFieldObservedAt.column:
		str = "observed_at"
	}
	return str
}
...
func (f *AssetHistoryOrderField) UnmarshalGQL(v interface{}) error {
	...
	case "observed_at":
		*f = *AssetHistoryOrderFieldObservedAt
	default:
...
// AudienceHistoryEdge is the edge representation of AudienceHistory.
type AudienceHistoryEdge struct {
	Node   *AudienceHistory `json:"node"`
	Cursor Cursor           `json:"cursor"`
}

// AudienceHistoryConnection is the connection containing edges to AudienceHistory.
type AudienceHistoryConnection struct {
	Edges      []*AudienceHistoryEdge `json:"edges"`
	PageInfo   PageInfo               `json:"pageInfo"`
	TotalCount int                    `json:"totalCount"`
}

func (c *AudienceHistoryConnection) build(nodes []*AudienceHistory, pager *audiencehistoryPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && len(nodes) >= *first+1 {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *AudienceHistory
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *AudienceHistory {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *AudienceHistory {
	...
	c.Edges = make([]*AudienceHistoryEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &AudienceHistoryEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// AudienceHistoryPaginateOption enables pagination customization.
type AudienceHistoryPaginateOption func(*audiencehistoryPager) error

// WithAudienceHistoryOrder configures pagination ordering.
func WithAudienceHistoryOrder(order *AudienceHistoryOrder) AudienceHistoryPaginateOption {
	if order == nil {
		order = DefaultAudienceHistoryOrder
	}
	o := *order
	return func(pager *audiencehistoryPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultAudienceHistoryOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithAudienceHistoryFilter configures pagination filter.
func WithAudienceHistoryFilter(filter func(*AudienceHistoryQuery) (*AudienceHistoryQuery, error)) AudienceHistoryPaginateOption {
	return func(pager *audiencehistoryPager) error {
		if filter == nil {
			return errors.New("AudienceHistoryQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type audiencehistoryPager struct {
	reverse bool
	order   *AudienceHistoryOrder
	filter  func(*AudienceHistoryQuery) (*AudienceHistoryQuery, error)
}

func newAudienceHistoryPager(opts []AudienceHistoryPaginateOption, reverse bool) (*audiencehistoryPager, error) {
	pager := &audiencehistoryPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultAudienceHistoryOrder
	}
	return pager, nil
}

func (p *audiencehistoryPager) applyFilter(query *AudienceHistoryQuery) (*AudienceHistoryQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *audiencehistoryPager) toCursor(_m *AudienceHistory) Cursor {
	return p.order.Field.toCursor(_m)
}

func (p *audiencehistoryPager) applyCursors(query *AudienceHistoryQuery, after, before *Cursor) (*AudienceHistoryQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultAudienceHistoryOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *audiencehistoryPager) applyOrder(query *AudienceHistoryQuery) *AudienceHistoryQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultAudienceHistoryOrder.Field {
		query = query.Order(DefaultAudienceHistoryOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *audiencehistoryPager) orderExpr(query *AudienceHistoryQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultAudienceHistoryOrder.Field {
			b.Comma().Ident(DefaultAudienceHistoryOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to AudienceHistory.
func (_m *AudienceHistoryQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...AudienceHistoryPaginateOption,
) (*AudienceHistoryConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newAudienceHistoryPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if _m, err = pager.applyFilter(_m); err != nil {
	...
	conn := &AudienceHistoryConnection{Edges: []*AudienceHistoryEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
...
var (
	// AudienceHistoryOrderFieldHistoryTime orders AudienceHistory by history_time.
	AudienceHistoryOrderFieldHistoryTime = &AudienceHistoryOrderField{
		Value: func(_m *AudienceHistory) (ent.Value, error) {
			return _m.HistoryTime, nil
		},
		column: audiencehistory.FieldHistoryTime,
		toTerm: audiencehistory.ByHistoryTime,
		toCursor: func(_m *AudienceHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.HistoryTime,
			}
	...
	// AudienceHistoryOrderFieldCreatedAt orders AudienceHistory by created_at.
	AudienceHistoryOrderFieldCreatedAt = &AudienceHistoryOrderField{
		Value: func(_m *AudienceHistory) (ent.Value, error) {
			return _m.CreatedAt, nil
		},
		column: audiencehistory.FieldCreatedAt,
		toTerm: audiencehistory.ByCreatedAt,
		toCursor: func(_m *AudienceHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.CreatedAt,
	...
	// AudienceHistoryOrderFieldUpdatedAt orders AudienceHistory by updated_at.
	AudienceHistoryOrderFieldUpdatedAt = &AudienceHistoryOrderField{
		Value: func(_m *AudienceHistory) (ent.Value, error) {
			return _m.UpdatedAt, nil
		},
		column: audiencehistory.FieldUpdatedAt,
		toTerm: audiencehistory.ByUpdatedAt,
		toCursor: func(_m *AudienceHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.UpdatedAt,
	...
	// AudienceHistoryOrderFieldName orders AudienceHistory by name.
	AudienceHistoryOrderFieldName = &AudienceHistoryOrderField{
		Value: func(_m *AudienceHistory) (ent.Value, error) {
			return _m.Name, nil
		},
		column: audiencehistory.FieldName,
		toTerm: audiencehistory.ByName,
		toCursor: func(_m *AudienceHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.Name,
	...
	// AudienceHistoryOrderFieldAudienceType orders AudienceHistory by audience_type.
	AudienceHistoryOrderFieldAudienceType = &AudienceHistoryOrderField{
		Value: func(_m *AudienceHistory) (ent.Value, error) {
			return _m.AudienceType, nil
		},
		column: audiencehistory.FieldAudienceType,
		toTerm: audiencehistory.ByAudienceType,
		toCursor: func(_m *AudienceHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
...
// String implement fmt.Stringer interface.
func (f AudienceHistoryOrderField) String() string {
	var str string
	switch f.column {
	case AudienceHistoryOrderFieldHistoryTime.column:
		str = "history_time"
	case AudienceHistoryOrderFieldCreatedAt.column:
		str = "created_at"
	case AudienceHistoryOrderFieldUpdatedAt.column:
		str = "updated_at"
	case AudienceHistoryOrderFieldName.column:
		str = "name"
	case AudienceHistoryOrderFieldAudienceType.column:
		str = "AUDIENCE_TYPE"
	}
	return str
}
...
// MarshalGQL implements graphql.Marshaler interface.
func (f AudienceHistoryOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *AudienceHistoryOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("AudienceHistoryOrderField %T must be a string", v)
	}
	switch str {
	case "history_time":
		*f = *AudienceHistoryOrderFieldHistoryTime
	case "created_at":
		*f = *AudienceHistoryOrderFieldCreatedAt
	case "updated_at":
		*f = *AudienceHistoryOrderFieldUpdatedAt
	case "name":
		*f = *AudienceHistoryOrderFieldName
	case "AUDIENCE_TYPE":
		*f = *AudienceHistoryOrderFieldAudienceType
	default:
		return fmt.Errorf("%s is not a valid AudienceHistoryOrderField", str)
	}
	return nil
}

// AudienceHistoryOrderField defines the ordering field of AudienceHistory.
type AudienceHistoryOrderField struct {
	// Value extracts the ordering value from the given AudienceHistory.
	Value    func(*AudienceHistory) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) audiencehistory.OrderOption
	toCursor func(*AudienceHistory) Cursor
}

// AudienceHistoryOrder defines the ordering of AudienceHistory.
type AudienceHistoryOrder struct {
	Direction OrderDirection             `json:"direction"`
	Field     *AudienceHistoryOrderField `json:"field"`
}

// DefaultAudienceHistoryOrder is the default ordering of AudienceHistory.
var DefaultAudienceHistoryOrder = &AudienceHistoryOrder{
	Direction: entgql.OrderDirectionAsc,
...
// ToEdge converts AudienceHistory into AudienceHistoryEdge.
func (_m *AudienceHistory) ToEdge(order *AudienceHistoryOrder) *AudienceHistoryEdge {
	if order == nil {
		order = DefaultAudienceHistoryOrder
	}
	return &AudienceHistoryEdge{
		Node:   _m,
		Cursor: order.Field.toCursor(_m),
	}
}

// AudienceMemberHistoryEdge is the edge representation of AudienceMemberHistory.
type AudienceMemberHistoryEdge struct {
	Node   *AudienceMemberHistory `json:"node"`
	Cursor Cursor                 `json:"cursor"`
}

// AudienceMemberHistoryConnection is the connection containing edges to AudienceMemberHistory.
type AudienceMemberHistoryConnection struct {
	Edges      []*AudienceMemberHistoryEdge `json:"edges"`
	PageInfo   PageInfo                     `json:"pageInfo"`
	TotalCount int                          `json:"totalCount"`
}

func (c *AudienceMemberHistoryConnection) build(nodes []*AudienceMemberHistory, pager *audiencememberhistoryPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && len(nodes) >= *first+1 {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *AudienceMemberHistory
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *AudienceMemberHistory {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *AudienceMemberHistory {
	...
	c.Edges = make([]*AudienceMemberHistoryEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &AudienceMemberHistoryEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// AudienceMemberHistoryPaginateOption enables pagination customization.
type AudienceMemberHistoryPaginateOption func(*audiencememberhistoryPager) error

// WithAudienceMemberHistoryOrder configures pagination ordering.
func WithAudienceMemberHistoryOrder(order *AudienceMemberHistoryOrder) AudienceMemberHistoryPaginateOption {
	if order == nil {
		order = DefaultAudienceMemberHistoryOrder
	}
	o := *order
	return func(pager *audiencememberhistoryPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultAudienceMemberHistoryOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithAudienceMemberHistoryFilter configures pagination filter.
func WithAudienceMemberHistoryFilter(filter func(*AudienceMemberHistoryQuery) (*AudienceMemberHistoryQuery, error)) AudienceMemberHistoryPaginateOption {
	return func(pager *audiencememberhistoryPager) error {
		if filter == nil {
			return errors.New("AudienceMemberHistoryQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type audiencememberhistoryPager struct {
	reverse bool
	order   *AudienceMemberHistoryOrder
	filter  func(*AudienceMemberHistoryQuery) (*AudienceMemberHistoryQuery, error)
}

func newAudienceMemberHistoryPager(opts []AudienceMemberHistoryPaginateOption, reverse bool) (*audiencememberhistoryPager, error) {
	pager := &audiencememberhistoryPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultAudienceMemberHistoryOrder
	}
	return pager, nil
}

func (p *audiencememberhistoryPager) applyFilter(query *AudienceMemberHistoryQuery) (*AudienceMemberHistoryQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *audiencememberhistoryPager) toCursor(_m *AudienceMemberHistory) Cursor {
	return p.order.Field.toCursor(_m)
}

func (p *audiencememberhistoryPager) applyCursors(query *AudienceMemberHistoryQuery, after, before *Cursor) (*AudienceMemberHistoryQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultAudienceMemberHistoryOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *audiencememberhistoryPager) applyOrder(query *AudienceMemberHistoryQuery) *AudienceMemberHistoryQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultAudienceMemberHistoryOrder.Field {
		query = query.Order(DefaultAudienceMemberHistoryOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *audiencememberhistoryPager) orderExpr(query *AudienceMemberHistoryQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultAudienceMemberHistoryOrder.Field {
			b.Comma().Ident(DefaultAudienceMemberHistoryOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to AudienceMemberHistory.
func (_m *AudienceMemberHistoryQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...AudienceMemberHistoryPaginateOption,
) (*AudienceMemberHistoryConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newAudienceMemberHistoryPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if _m, err = pager.applyFilter(_m); err != nil {
	...
	conn := &AudienceMemberHistoryConnection{Edges: []*AudienceMemberHistoryEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
	...
	_m = pager.applyOrder(_m)
	var nodes []*AudienceMemberHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
var (
	// AudienceMemberHistoryOrderFieldHistoryTime orders AudienceMemberHistory by history_time.
	AudienceMemberHistoryOrderFieldHistoryTime = &AudienceMemberHistoryOrderField{
		Value: func(_m *AudienceMemberHistory) (ent.Value, error) {
			return _m.HistoryTime, nil
		},
		column: audiencememberhistory.FieldHistoryTime,
		toTerm: audiencememberhistory.ByHistoryTime,
		toCursor: func(_m *AudienceMemberHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.HistoryTime,
			}
	...
	// AudienceMemberHistoryOrderFieldCreatedAt orders AudienceMemberHistory by created_at.
	AudienceMemberHistoryOrderFieldCreatedAt = &AudienceMemberHistoryOrderField{
		Value: func(_m *AudienceMemberHistory) (ent.Value, error) {
			return _m.CreatedAt, nil
		},
		column: audiencememberhistory.FieldCreatedAt,
		toTerm: audiencememberhistory.ByCreatedAt,
		toCursor: func(_m *AudienceMemberHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.CreatedAt,
	...
	// AudienceMemberHistoryOrderFieldUpdatedAt orders AudienceMemberHistory by updated_at.
	AudienceMemberHistoryOrderFieldUpdatedAt = &AudienceMemberHistoryOrderField{
		Value: func(_m *AudienceMemberHistory) (ent.Value, error) {
			return _m.UpdatedAt, nil
		},
		column: audiencememberhistory.FieldUpdatedAt,
		toTerm: audiencememberhistory.ByUpdatedAt,
		toCursor: func(_m *AudienceMemberHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.UpdatedAt,
	...
	// AudienceMemberHistoryOrderFieldEmail orders AudienceMemberHistory by email.
	AudienceMemberHistoryOrderFieldEmail = &AudienceMemberHistoryOrderField{
		Value: func(_m *AudienceMemberHistory) (ent.Value, error) {
			return _m.Email, nil
		},
		column: audiencememberhistory.FieldEmail,
		toTerm: audiencememberhistory.ByEmail,
		toCursor: func(_m *AudienceMemberHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.Email,
	...
	// AudienceMemberHistoryOrderFieldFullName orders AudienceMemberHistory by full_name.
	AudienceMemberHistoryOrderFieldFullName = &AudienceMemberHistoryOrderField{
		Value: func(_m *AudienceMemberHistory) (ent.Value, error) {
			return _m.FullName, nil
		},
		column: audiencememberhistory.FieldFullName,
		toTerm: audiencememberhistory.ByFullName,
		toCursor: func(_m *AudienceMemberHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.FullName,
...
// String implement fmt.Stringer interface.
func (f AudienceMemberHistoryOrderField) String() string {
	var str string
	switch f.column {
	case AudienceMemberHistoryOrderFieldHistoryTime.column:
		str = "history_time"
	case AudienceMemberHistoryOrderFieldCreatedAt.column:
		str = "created_at"
	case AudienceMemberHistoryOrderFieldUpdatedAt.column:
		str = "updated_at"
	case AudienceMemberHistoryOrderFieldEmail.column:
		str = "email"
	case AudienceMemberHistoryOrderFieldFullName.column:
		str = "full_name"
	}
	return str
}
...
// MarshalGQL implements graphql.Marshaler interface.
func (f AudienceMemberHistoryOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *AudienceMemberHistoryOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("AudienceMemberHistoryOrderField %T must be a string", v)
	}
	switch str {
	case "history_time":
		*f = *AudienceMemberHistoryOrderFieldHistoryTime
	case "created_at":
		*f = *AudienceMemberHistoryOrderFieldCreatedAt
	case "updated_at":
		*f = *AudienceMemberHistoryOrderFieldUpdatedAt
	case "email":
		*f = *AudienceMemberHistoryOrderFieldEmail
	case "full_name":
		*f = *AudienceMemberHistoryOrderFieldFullName
	default:
		return fmt.Errorf("%s is not a valid AudienceMemberHistoryOrderField", str)
	}
	return nil
}

// AudienceMemberHistoryOrderField defines the ordering field of AudienceMemberHistory.
type AudienceMemberHistoryOrderField struct {
	// Value extracts the ordering value from the given AudienceMemberHistory.
	Value    func(*AudienceMemberHistory) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) audiencememberhistory.OrderOption
	toCursor func(*AudienceMemberHistory) Cursor
}

// AudienceMemberHistoryOrder defines the ordering of AudienceMemberHistory.
type AudienceMemberHistoryOrder struct {
	Direction OrderDirection                   `json:"direction"`
	Field     *AudienceMemberHistoryOrderField `json:"field"`
}

// DefaultAudienceMemberHistoryOrder is the default ordering of AudienceMemberHistory.
var DefaultAudienceMemberHistoryOrder = &AudienceMemberHistoryOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &AudienceMemberHistoryOrderField{
		...
		toTerm: audiencememberhistory.ByID,
		toCursor: func(_m *AudienceMemberHistory) Cursor {
			return Cursor{ID: _m.ID}
		},
	},
...
// ToEdge converts AudienceMemberHistory into AudienceMemberHistoryEdge.
func (_m *AudienceMemberHistory) ToEdge(order *AudienceMemberHistoryOrder) *AudienceMemberHistoryEdge {
	if order == nil {
		order = DefaultAudienceMemberHistoryOrder
	}
	return &AudienceMemberHistoryEdge{
		Node:   _m,
		Cursor: order.Field.toCursor(_m),
	}
}
...
func (_m *CampaignTargetHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*CampaignTargetHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
var (
	...
	// ContactHistoryOrderFieldExternalID orders ContactHistory by external_id.
	ContactHistoryOrderFieldExternalID = &ContactHistoryOrderField{
		Value: func(_m *ContactHistory) (ent.Value, error) {
			return _m.ExternalID, nil
		},
		column: contacthistory.FieldExternalID,
		toTerm: contacthistory.ByExternalID,
		toCursor: func(_m *ContactHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.ExternalID,
	...
	// ContactHistoryOrderFieldObservedAt orders ContactHistory by observed_at.
	ContactHistoryOrderFieldObservedAt = &ContactHistoryOrderField{
		Value: func(_m *ContactHistory) (ent.Value, error) {
			// allow for nil values for fields
			if _m.ObservedAt == nil {
				return nil, nil
		...
		column: contacthistory.FieldObservedAt,
		toTerm: func(opts ...sql.OrderTermOption) contacthistory.OrderOption {
			opts = append(opts, sql.OrderNullsLast())
			return contacthistory.ByObservedAt(opts...)
		},
		toCursor: func(_m *ContactHistory) Cursor {
			if _m.ObservedAt == nil {
				return Cursor{
...
func (f ContactHistoryOrderField) String() string {
	...
	case ContactHistoryOrderFieldStatus.column:
		str = "STATUS"
	case ContactHistoryOrderFieldExternalID.column:
		str = "external_id"
	case ContactHistoryOrderFieldObservedAt.column:
		str = "observed_at"
	}
	return str
}
...
func (f *ContactHistoryOrderField) UnmarshalGQL(v interface{}) error {
	...
	case "external_id":
		*f = *ContactHistoryOrderFieldExternalID
	case "observed_at":
		*f = *ContactHistoryOrderFieldObservedAt
	default:
...
func (_m *ControlImplementationHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*ControlImplementationHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *ControlObjectiveHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*ControlObjectiveHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *EmailTemplateHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*EmailTemplateHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
var (
	...
	// EntityHistoryOrderFieldRiskScoreCoverage orders EntityHistory by risk_score_coverage.
	EntityHistoryOrderFieldRiskScoreCoverage = &EntityHistoryOrderField{
		Value: func(_m *EntityHistory) (ent.Value, error) {
			return _m.RiskScoreCoverage, nil
		},
		column: entityhistory.FieldRiskScoreCoverage,
		toTerm: entityhistory.ByRiskScoreCoverage,
		toCursor: func(_m *EntityHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
	...
	// EntityHistoryOrderFieldExternalID orders EntityHistory by external_id.
	EntityHistoryOrderFieldExternalID = &EntityHistoryOrderField{
		Value: func(_m *EntityHistory) (ent.Value, error) {
			return _m.ExternalID, nil
		},
		column: entityhistory.FieldExternalID,
		toTerm: entityhistory.ByExternalID,
		toCursor: func(_m *EntityHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.ExternalID,
	...
	// EntityHistoryOrderFieldObservedAt orders EntityHistory by observed_at.
	EntityHistoryOrderFieldObservedAt = &EntityHistoryOrderField{
		Value: func(_m *EntityHistory) (ent.Value, error) {
			// allow for nil values for fields
			if _m.ObservedAt == nil {
				return nil, nil
		...
		column: entityhistory.FieldObservedAt,
		toTerm: func(opts ...sql.OrderTermOption) entityhistory.OrderOption {
			opts = append(opts, sql.OrderNullsLast())
			return entityhistory.ByObservedAt(opts...)
		},
		toCursor: func(_m *EntityHistory) Cursor {
			if _m.ObservedAt == nil {
				return Cursor{
...
func (f EntityHistoryOrderField) String() string {
	...
	case EntityHistoryOrderFieldRiskScore.column:
		str = "risk_score"
	case EntityHistoryOrderFieldRiskScoreCoverage.column:
	...
	case EntityHistoryOrderFieldExternalID.column:
		str = "external_id"
	case EntityHistoryOrderFieldObservedAt.column:
		str = "observed_at"
	}
	return str
}
...
func (f *EntityHistoryOrderField) UnmarshalGQL(v interface{}) error {
	...
	case "risk_score_coverage":
		*f = *EntityHistoryOrderFieldRiskScoreCoverage
	case "tier":
	...
	case "external_id":
		*f = *EntityHistoryOrderFieldExternalID
	case "observed_at":
		*f = *EntityHistoryOrderFieldObservedAt
	default:
...
func (_m *FindingControlHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*FindingControlHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *GroupMembershipHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*GroupMembershipHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *IdentityHolderHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*IdentityHolderHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *InternalPolicyHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*InternalPolicyHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *MappableDomainHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*MappableDomainHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *MappedControlHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*MappedControlHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *NotificationPreferenceHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*NotificationPreferenceHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *NotificationTemplateHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*NotificationTemplateHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *OrgMembershipHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*OrgMembershipHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *OrganizationSettingHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*OrganizationSettingHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *ProgramMembershipHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*ProgramMembershipHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
var (
	...
	// RemediationHistoryOrderFieldStatus orders RemediationHistory by status.
	RemediationHistoryOrderFieldStatus = &RemediationHistoryOrderField{
		Value: func(_m *RemediationHistory) (ent.Value, error) {
			return _m.Status, nil
		},
		column: remediationhistory.FieldStatus,
		toTerm: remediationhistory.ByStatus,
		toCursor: func(_m *RemediationHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.Status,
...
func (f RemediationHistoryOrderField) String() string {
	...
	case RemediationHistoryOrderFieldTitle.column:
		str = "title"
	case RemediationHistoryOrderFieldStatus.column:
		str = "status"
...
func (f *RemediationHistoryOrderField) UnmarshalGQL(v interface{}) error {
	...
	case "status":
		*f = *RemediationHistoryOrderFieldStatus
	case "state":
...
var (
	...
	RiskHistoryOrderFieldExternalID = &RiskHistoryOrderField{
		...
		column: riskhistory.FieldExternalID,
		toTerm: riskhistory.ByExternalID,
		toCursor: func(_m *RiskHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.ExternalID,
	...
	// RiskHistoryOrderFieldObservedAt orders RiskHistory by observed_at.
	RiskHistoryOrderFieldObservedAt = &RiskHistoryOrderField{
		Value: func(_m *RiskHistory) (ent.Value, error) {
			// allow for nil values for fields
			if _m.ObservedAt == nil {
				return nil, nil
		...
		column: riskhistory.FieldObservedAt,
		toTerm: func(opts ...sql.OrderTermOption) riskhistory.OrderOption {
			opts = append(opts, sql.OrderNullsLast())
			return riskhistory.ByObservedAt(opts...)
		},
		toCursor: func(_m *RiskHistory) Cursor {
			if _m.ObservedAt == nil {
				return Cursor{
	...
	// RiskHistoryOrderFieldMitigatedAt orders RiskHistory by mitigated_at.
	RiskHistoryOrderFieldMitigatedAt = &RiskHistoryOrderField{
		Value: func(_m *RiskHistory) (ent.Value, error) {
			// allow for nil values for fields
		...
		column: riskhistory.FieldMitigatedAt,
		toTerm: func(opts ...sql.OrderTermOption) riskhistory.OrderOption {
			opts = append(opts, sql.OrderNullsLast())
			return riskhistory.ByMitigatedAt(opts...)
		},
		toCursor: func(_m *RiskHistory) Cursor {
			if _m.MitigatedAt == nil {
				return Cursor{
	...
	// RiskHistoryOrderFieldReviewRequired orders RiskHistory by review_required.
	RiskHistoryOrderFieldReviewRequired = &RiskHistoryOrderField{
		Value: func(_m *RiskHistory) (ent.Value, error) {
			return _m.ReviewRequired, nil
		},
		column: riskhistory.FieldReviewRequired,
		toTerm: riskhistory.ByReviewRequired,
		toCursor: func(_m *RiskHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
	...
	// RiskHistoryOrderFieldLastReviewedAt orders RiskHistory by last_reviewed_at.
	RiskHistoryOrderFieldLastReviewedAt = &RiskHistoryOrderField{
		Value: func(_m *RiskHistory) (ent.Value, error) {
			// allow for nil values for fields
			if _m.LastReviewedAt == nil {
				return nil, nil
		...
		column: riskhistory.FieldLastReviewedAt,
		toTerm: func(opts ...sql.OrderTermOption) riskhistory.OrderOption {
			opts = append(opts, sql.OrderNullsLast())
			return riskhistory.ByLastReviewedAt(opts...)
		},
		toCursor: func(_m *RiskHistory) Cursor {
			if _m.LastReviewedAt == nil {
				return Cursor{
	...
	// RiskHistoryOrderFieldReviewFrequency orders RiskHistory by review_frequency.
	RiskHistoryOrderFieldReviewFrequency = &RiskHistoryOrderField{
		Value: func(_m *RiskHistory) (ent.Value, error) {
			return _m.ReviewFrequency, nil
		},
		column: riskhistory.FieldReviewFrequency,
		toTerm: riskhistory.ByReviewFrequency,
		toCursor: func(_m *RiskHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.ReviewFrequency,
	...
	RiskHistoryOrderFieldNextReviewDueAt = &RiskHistoryOrderField{
		...
		column: riskhistory.FieldNextReviewDueAt,
		toTerm: func(opts ...sql.OrderTermOption) riskhistory.OrderOption {
			opts = append(opts, sql.OrderNullsLast())
			return riskhistory.ByNextReviewDueAt(opts...)
		},
		toCursor: func(_m *RiskHistory) Cursor {
			if _m.NextReviewDueAt == nil {
				return Cursor{
	...
	// RiskHistoryOrderFieldResidualScore orders RiskHistory by residual_score.
	RiskHistoryOrderFieldResidualScore = &RiskHistoryOrderField{
		Value: func(_m *RiskHistory) (ent.Value, error) {
			return _m.ResidualScore, nil
		},
		column: riskhistory.FieldResidualScore,
		toTerm: riskhistory.ByResidualScore,
		toCursor: func(_m *RiskHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
	...
	// RiskHistoryOrderFieldRiskDecision orders RiskHistory by risk_decision.
	RiskHistoryOrderFieldRiskDecision = &RiskHistoryOrderField{
		Value: func(_m *RiskHistory) (ent.Value, error) {
			return _m.RiskDecision, nil
		},
		column: riskhistory.FieldRiskDecision,
		toTerm: riskhistory.ByRiskDecision,
		toCursor: func(_m *RiskHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
...
func (f RiskHistoryOrderField) String() string {
	...
	case RiskHistoryOrderFieldBusinessCosts.column:
		str = "business_costs"
	case RiskHistoryOrderFieldMitigatedAt.column:
		str = "mitigated_at"
	case RiskHistoryOrderFieldReviewRequired.column:
		str = "review_required"
	case RiskHistoryOrderFieldLastReviewedAt.column:
		str = "last_reviewed_at"
	...
	case RiskHistoryOrderFieldResidualScore.column:
		str = "residual_score"
	case RiskHistoryOrderFieldRiskDecision.column:
		str = "risk_decision"
	}
	return str
}
...
func (f *RiskHistoryOrderField) UnmarshalGQL(v interface{}) error {
	...
	case "observed_at":
		*f = *RiskHistoryOrderFieldObservedAt
	case "name":
	...
	case "mitigated_at":
		*f = *RiskHistoryOrderFieldMitigatedAt
	case "review_required":
		*f = *RiskHistoryOrderFieldReviewRequired
	case "last_reviewed_at":
	...
	case "residual_score":
		*f = *RiskHistoryOrderFieldResidualScore
	case "risk_decision":
		*f = *RiskHistoryOrderFieldRiskDecision
	default:
...
func (_m *SLADefinitionHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*SLADefinitionHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *TrustCenterComplianceHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*TrustCenterComplianceHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *TrustCenterDocHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*TrustCenterDocHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *TrustCenterEntityHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*TrustCenterEntityHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *TrustCenterFAQHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*TrustCenterFAQHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *TrustCenterNDARequestHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*TrustCenterNDARequestHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *TrustCenterSettingHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*TrustCenterSettingHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *TrustCenterSubprocessorHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*TrustCenterSubprocessorHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *TrustCenterWatermarkConfigHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*TrustCenterWatermarkConfigHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
// VendorRiskScoreHistoryEdge is the edge representation of VendorRiskScoreHistory.
type VendorRiskScoreHistoryEdge struct {
	Node   *VendorRiskScoreHistory `json:"node"`
	Cursor Cursor                  `json:"cursor"`
}

// VendorRiskScoreHistoryConnection is the connection containing edges to VendorRiskScoreHistory.
type VendorRiskScoreHistoryConnection struct {
	Edges      []*VendorRiskScoreHistoryEdge `json:"edges"`
	PageInfo   PageInfo                      `json:"pageInfo"`
	TotalCount int                           `json:"totalCount"`
}

func (c *VendorRiskScoreHistoryConnection) build(nodes []*VendorRiskScoreHistory, pager *vendorriskscorehistoryPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && len(nodes) >= *first+1 {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *VendorRiskScoreHistory
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *VendorRiskScoreHistory {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *VendorRiskScoreHistory {
	...
	c.Edges = make([]*VendorRiskScoreHistoryEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &VendorRiskScoreHistoryEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// VendorRiskScoreHistoryPaginateOption enables pagination customization.
type VendorRiskScoreHistoryPaginateOption func(*vendorriskscorehistoryPager) error

// WithVendorRiskScoreHistoryOrder configures pagination ordering.
func WithVendorRiskScoreHistoryOrder(order *VendorRiskScoreHistoryOrder) VendorRiskScoreHistoryPaginateOption {
	if order == nil {
		order = DefaultVendorRiskScoreHistoryOrder
	}
	o := *order
	return func(pager *vendorriskscorehistoryPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultVendorRiskScoreHistoryOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithVendorRiskScoreHistoryFilter configures pagination filter.
func WithVendorRiskScoreHistoryFilter(filter func(*VendorRiskScoreHistoryQuery) (*VendorRiskScoreHistoryQuery, error)) VendorRiskScoreHistoryPaginateOption {
	return func(pager *vendorriskscorehistoryPager) error {
		if filter == nil {
			return errors.New("VendorRiskScoreHistoryQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type vendorriskscorehistoryPager struct {
	reverse bool
	order   *VendorRiskScoreHistoryOrder
	filter  func(*VendorRiskScoreHistoryQuery) (*VendorRiskScoreHistoryQuery, error)
}

func newVendorRiskScoreHistoryPager(opts []VendorRiskScoreHistoryPaginateOption, reverse bool) (*vendorriskscorehistoryPager, error) {
	pager := &vendorriskscorehistoryPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultVendorRiskScoreHistoryOrder
	}
	return pager, nil
}

func (p *vendorriskscorehistoryPager) applyFilter(query *VendorRiskScoreHistoryQuery) (*VendorRiskScoreHistoryQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *vendorriskscorehistoryPager) toCursor(_m *VendorRiskScoreHistory) Cursor {
	return p.order.Field.toCursor(_m)
}

func (p *vendorriskscorehistoryPager) applyCursors(query *VendorRiskScoreHistoryQuery, after, before *Cursor) (*VendorRiskScoreHistoryQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultVendorRiskScoreHistoryOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *vendorriskscorehistoryPager) applyOrder(query *VendorRiskScoreHistoryQuery) *VendorRiskScoreHistoryQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultVendorRiskScoreHistoryOrder.Field {
		query = query.Order(DefaultVendorRiskScoreHistoryOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *vendorriskscorehistoryPager) orderExpr(query *VendorRiskScoreHistoryQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultVendorRiskScoreHistoryOrder.Field {
			b.Comma().Ident(DefaultVendorRiskScoreHistoryOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to VendorRiskScoreHistory.
func (_m *VendorRiskScoreHistoryQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...VendorRiskScoreHistoryPaginateOption,
) (*VendorRiskScoreHistoryConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newVendorRiskScoreHistoryPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if _m, err = pager.applyFilter(_m); err != nil {
	...
	conn := &VendorRiskScoreHistoryConnection{Edges: []*VendorRiskScoreHistoryEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
	...
	_m = pager.applyOrder(_m)
	var nodes []*VendorRiskScoreHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
var (
	// VendorRiskScoreHistoryOrderFieldHistoryTime orders VendorRiskScoreHistory by history_time.
	VendorRiskScoreHistoryOrderFieldHistoryTime = &VendorRiskScoreHistoryOrderField{
		Value: func(_m *VendorRiskScoreHistory) (ent.Value, error) {
			return _m.HistoryTime, nil
		},
		column: vendorriskscorehistory.FieldHistoryTime,
		toTerm: vendorriskscorehistory.ByHistoryTime,
		toCursor: func(_m *VendorRiskScoreHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.HistoryTime,
			}
	...
	// VendorRiskScoreHistoryOrderFieldCreatedAt orders VendorRiskScoreHistory by created_at.
	VendorRiskScoreHistoryOrderFieldCreatedAt = &VendorRiskScoreHistoryOrderField{
		Value: func(_m *VendorRiskScoreHistory) (ent.Value, error) {
			return _m.CreatedAt, nil
		},
		column: vendorriskscorehistory.FieldCreatedAt,
		toTerm: vendorriskscorehistory.ByCreatedAt,
		toCursor: func(_m *VendorRiskScoreHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.CreatedAt,
	...
	// VendorRiskScoreHistoryOrderFieldUpdatedAt orders VendorRiskScoreHistory by updated_at.
	VendorRiskScoreHistoryOrderFieldUpdatedAt = &VendorRiskScoreHistoryOrderField{
		Value: func(_m *VendorRiskScoreHistory) (ent.Value, error) {
			return _m.UpdatedAt, nil
		},
		column: vendorriskscorehistory.FieldUpdatedAt,
		toTerm: vendorriskscorehistory.ByUpdatedAt,
		toCursor: func(_m *VendorRiskScoreHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.UpdatedAt,
	...
	// VendorRiskScoreHistoryOrderFieldQuestionKey orders VendorRiskScoreHistory by question_key.
	VendorRiskScoreHistoryOrderFieldQuestionKey = &VendorRiskScoreHistoryOrderField{
		Value: func(_m *VendorRiskScoreHistory) (ent.Value, error) {
			return _m.QuestionKey, nil
		},
		column: vendorriskscorehistory.FieldQuestionKey,
		toTerm: vendorriskscorehistory.ByQuestionKey,
		toCursor: func(_m *VendorRiskScoreHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
	...
	// VendorRiskScoreHistoryOrderFieldQuestionCategory orders VendorRiskScoreHistory by question_category.
	VendorRiskScoreHistoryOrderFieldQuestionCategory = &VendorRiskScoreHistoryOrderField{
		Value: func(_m *VendorRiskScoreHistory) (ent.Value, error) {
			return _m.QuestionCategory, nil
		},
		column: vendorriskscorehistory.FieldQuestionCategory,
		toTerm: vendorriskscorehistory.ByQuestionCategory,
		toCursor: func(_m *VendorRiskScoreHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
	...
	// VendorRiskScoreHistoryOrderFieldImpact orders VendorRiskScoreHistory by impact.
	VendorRiskScoreHistoryOrderFieldImpact = &VendorRiskScoreHistoryOrderField{
		Value: func(_m *VendorRiskScoreHistory) (ent.Value, error) {
			return _m.Impact, nil
		},
		column: vendorriskscorehistory.FieldImpact,
		toTerm: vendorriskscorehistory.ByImpact,
		toCursor: func(_m *VendorRiskScoreHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.Impact,
	...
	// VendorRiskScoreHistoryOrderFieldLikelihood orders VendorRiskScoreHistory by likelihood.
	VendorRiskScoreHistoryOrderFieldLikelihood = &VendorRiskScoreHistoryOrderField{
		Value: func(_m *VendorRiskScoreHistory) (ent.Value, error) {
			return _m.Likelihood, nil
		},
		column: vendorriskscorehistory.FieldLikelihood,
		toTerm: vendorriskscorehistory.ByLikelihood,
		toCursor: func(_m *VendorRiskScoreHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.Likelihood,
	...
	// VendorRiskScoreHistoryOrderFieldScore orders VendorRiskScoreHistory by score.
	VendorRiskScoreHistoryOrderFieldScore = &VendorRiskScoreHistoryOrderField{
		Value: func(_m *VendorRiskScoreHistory) (ent.Value, error) {
			return _m.Score, nil
		},
		column: vendorriskscorehistory.FieldScore,
		toTerm: vendorriskscorehistory.ByScore,
		toCursor: func(_m *VendorRiskScoreHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.Score,
...
// String implement fmt.Stringer interface.
func (f VendorRiskScoreHistoryOrderField) String() string {
	var str string
	switch f.column {
	case VendorRiskScoreHistoryOrderFieldHistoryTime.column:
		str = "history_time"
	case VendorRiskScoreHistoryOrderFieldCreatedAt.column:
		str = "created_at"
	case VendorRiskScoreHistoryOrderFieldUpdatedAt.column:
		str = "updated_at"
	case VendorRiskScoreHistoryOrderFieldQuestionKey.column:
		str = "question_key"
	case VendorRiskScoreHistoryOrderFieldQuestionCategory.column:
		str = "question_category"
	case VendorRiskScoreHistoryOrderFieldImpact.column:
		str = "impact"
	case VendorRiskScoreHistoryOrderFieldLikelihood.column:
		str = "likelihood"
	case VendorRiskScoreHistoryOrderFieldScore.column:
		str = "score"
	}
	return str
}
...
// MarshalGQL implements graphql.Marshaler interface.
func (f VendorRiskScoreHistoryOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *VendorRiskScoreHistoryOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("VendorRiskScoreHistoryOrderField %T must be a string", v)
	}
	switch str {
	case "history_time":
		*f = *VendorRiskScoreHistoryOrderFieldHistoryTime
	case "created_at":
		*f = *VendorRiskScoreHistoryOrderFieldCreatedAt
	case "updated_at":
		*f = *VendorRiskScoreHistoryOrderFieldUpdatedAt
	case "question_key":
		*f = *VendorRiskScoreHistoryOrderFieldQuestionKey
	case "question_category":
		*f = *VendorRiskScoreHistoryOrderFieldQuestionCategory
	case "impact":
		*f = *VendorRiskScoreHistoryOrderFieldImpact
	case "likelihood":
		*f = *VendorRiskScoreHistoryOrderFieldLikelihood
	case "score":
		*f = *VendorRiskScoreHistoryOrderFieldScore
	default:
		return fmt.Errorf("%s is not a valid VendorRiskScoreHistoryOrderField", str)
	}
	return nil
}

// VendorRiskScoreHistoryOrderField defines the ordering field of VendorRiskScoreHistory.
type VendorRiskScoreHistoryOrderField struct {
	// Value extracts the ordering value from the given VendorRiskScoreHistory.
	Value    func(*VendorRiskScoreHistory) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) vendorriskscorehistory.OrderOption
	toCursor func(*VendorRiskScoreHistory) Cursor
}

// VendorRiskScoreHistoryOrder defines the ordering of VendorRiskScoreHistory.
type VendorRiskScoreHistoryOrder struct {
	Direction OrderDirection                    `json:"direction"`
	Field     *VendorRiskScoreHistoryOrderField `json:"field"`
}

// DefaultVendorRiskScoreHistoryOrder is the default ordering of VendorRiskScoreHistory.
var DefaultVendorRiskScoreHistoryOrder = &VendorRiskScoreHistoryOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &VendorRiskScoreHistoryOrderField{
		...
		toTerm: vendorriskscorehistory.ByID,
		toCursor: func(_m *VendorRiskScoreHistory) Cursor {
			return Cursor{ID: _m.ID}
		},
	},
...
// ToEdge converts VendorRiskScoreHistory into VendorRiskScoreHistoryEdge.
func (_m *VendorRiskScoreHistory) ToEdge(order *VendorRiskScoreHistoryOrder) *VendorRiskScoreHistoryEdge {
	if order == nil {
		order = DefaultVendorRiskScoreHistoryOrder
	}
	return &VendorRiskScoreHistoryEdge{
		Node:   _m,
		Cursor: order.Field.toCursor(_m),
	}
}

// VendorScoringConfigHistoryEdge is the edge representation of VendorScoringConfigHistory.
type VendorScoringConfigHistoryEdge struct {
	Node   *VendorScoringConfigHistory `json:"node"`
	Cursor Cursor                      `json:"cursor"`
}

// VendorScoringConfigHistoryConnection is the connection containing edges to VendorScoringConfigHistory.
type VendorScoringConfigHistoryConnection struct {
	Edges      []*VendorScoringConfigHistoryEdge `json:"edges"`
	PageInfo   PageInfo                          `json:"pageInfo"`
	TotalCount int                               `json:"totalCount"`
}

func (c *VendorScoringConfigHistoryConnection) build(nodes []*VendorScoringConfigHistory, pager *vendorscoringconfighistoryPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && len(nodes) >= *first+1 {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *VendorScoringConfigHistory
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *VendorScoringConfigHistory {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *VendorScoringConfigHistory {
	...
	c.Edges = make([]*VendorScoringConfigHistoryEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &VendorScoringConfigHistoryEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// VendorScoringConfigHistoryPaginateOption enables pagination customization.
type VendorScoringConfigHistoryPaginateOption func(*vendorscoringconfighistoryPager) error

// WithVendorScoringConfigHistoryOrder configures pagination ordering.
func WithVendorScoringConfigHistoryOrder(order *VendorScoringConfigHistoryOrder) VendorScoringConfigHistoryPaginateOption {
	if order == nil {
		order = DefaultVendorScoringConfigHistoryOrder
	}
	o := *order
	return func(pager *vendorscoringconfighistoryPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultVendorScoringConfigHistoryOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithVendorScoringConfigHistoryFilter configures pagination filter.
func WithVendorScoringConfigHistoryFilter(filter func(*VendorScoringConfigHistoryQuery) (*VendorScoringConfigHistoryQuery, error)) VendorScoringConfigHistoryPaginateOption {
	return func(pager *vendorscoringconfighistoryPager) error {
		if filter == nil {
			return errors.New("VendorScoringConfigHistoryQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type vendorscoringconfighistoryPager struct {
	reverse bool
	order   *VendorScoringConfigHistoryOrder
	filter  func(*VendorScoringConfigHistoryQuery) (*VendorScoringConfigHistoryQuery, error)
}

func newVendorScoringConfigHistoryPager(opts []VendorScoringConfigHistoryPaginateOption, reverse bool) (*vendorscoringconfighistoryPager, error) {
	pager := &vendorscoringconfighistoryPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultVendorScoringConfigHistoryOrder
	}
	return pager, nil
}

func (p *vendorscoringconfighistoryPager) applyFilter(query *VendorScoringConfigHistoryQuery) (*VendorScoringConfigHistoryQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *vendorscoringconfighistoryPager) toCursor(_m *VendorScoringConfigHistory) Cursor {
	return p.order.Field.toCursor(_m)
}

func (p *vendorscoringconfighistoryPager) applyCursors(query *VendorScoringConfigHistoryQuery, after, before *Cursor) (*VendorScoringConfigHistoryQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultVendorScoringConfigHistoryOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *vendorscoringconfighistoryPager) applyOrder(query *VendorScoringConfigHistoryQuery) *VendorScoringConfigHistoryQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultVendorScoringConfigHistoryOrder.Field {
		query = query.Order(DefaultVendorScoringConfigHistoryOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *vendorscoringconfighistoryPager) orderExpr(query *VendorScoringConfigHistoryQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultVendorScoringConfigHistoryOrder.Field {
			b.Comma().Ident(DefaultVendorScoringConfigHistoryOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to VendorScoringConfigHistory.
func (_m *VendorScoringConfigHistoryQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...VendorScoringConfigHistoryPaginateOption,
) (*VendorScoringConfigHistoryConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newVendorScoringConfigHistoryPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if _m, err = pager.applyFilter(_m); err != nil {
	...
	conn := &VendorScoringConfigHistoryConnection{Edges: []*VendorScoringConfigHistoryEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
	...
	_m = pager.applyOrder(_m)
	var nodes []*VendorScoringConfigHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
var (
	...
	VendorScoringConfigHistoryOrderFieldHistoryTime = &VendorScoringConfigHistoryOrderField{
		Value: func(_m *VendorScoringConfigHistory) (ent.Value, error) {
			return _m.HistoryTime, nil
		},
		column: vendorscoringconfighistory.FieldHistoryTime,
		toTerm: vendorscoringconfighistory.ByHistoryTime,
		toCursor: func(_m *VendorScoringConfigHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.HistoryTime,
			}
	...
	// VendorScoringConfigHistoryOrderFieldCreatedAt orders VendorScoringConfigHistory by created_at.
	VendorScoringConfigHistoryOrderFieldCreatedAt = &VendorScoringConfigHistoryOrderField{
		Value: func(_m *VendorScoringConfigHistory) (ent.Value, error) {
			return _m.CreatedAt, nil
		},
		column: vendorscoringconfighistory.FieldCreatedAt,
		toTerm: vendorscoringconfighistory.ByCreatedAt,
		toCursor: func(_m *VendorScoringConfigHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.CreatedAt,
	...
	// VendorScoringConfigHistoryOrderFieldUpdatedAt orders VendorScoringConfigHistory by updated_at.
	VendorScoringConfigHistoryOrderFieldUpdatedAt = &VendorScoringConfigHistoryOrderField{
		Value: func(_m *VendorScoringConfigHistory) (ent.Value, error) {
			return _m.UpdatedAt, nil
		},
		column: vendorscoringconfighistory.FieldUpdatedAt,
		toTerm: vendorscoringconfighistory.ByUpdatedAt,
		toCursor: func(_m *VendorScoringConfigHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.UpdatedAt,
	...
	// VendorScoringConfigHistoryOrderFieldScoringMode orders VendorScoringConfigHistory by scoring_mode.
	VendorScoringConfigHistoryOrderFieldScoringMode = &VendorScoringConfigHistoryOrderField{
		Value: func(_m *VendorScoringConfigHistory) (ent.Value, error) {
			return _m.ScoringMode, nil
		},
		column: vendorscoringconfighistory.FieldScoringMode,
		toTerm: vendorscoringconfighistory.ByScoringMode,
		toCursor: func(_m *VendorScoringConfigHistory) Cursor {
			return Cursor{
				ID:    _m.ID,
...
// String implement fmt.Stringer interface.
func (f VendorScoringConfigHistoryOrderField) String() string {
	var str string
	switch f.column {
	case VendorScoringConfigHistoryOrderFieldHistoryTime.column:
		str = "history_time"
	case VendorScoringConfigHistoryOrderFieldCreatedAt.column:
		str = "created_at"
	case VendorScoringConfigHistoryOrderFieldUpdatedAt.column:
		str = "updated_at"
	case VendorScoringConfigHistoryOrderFieldScoringMode.column:
		str = "scoring_mode"
	}
	return str
}
...
// MarshalGQL implements graphql.Marshaler interface.
func (f VendorScoringConfigHistoryOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *VendorScoringConfigHistoryOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("VendorScoringConfigHistoryOrderField %T must be a string", v)
	}
	switch str {
	case "history_time":
		*f = *VendorScoringConfigHistoryOrderFieldHistoryTime
	case "created_at":
		*f = *VendorScoringConfigHistoryOrderFieldCreatedAt
	case "updated_at":
		*f = *VendorScoringConfigHistoryOrderFieldUpdatedAt
	case "scoring_mode":
		*f = *VendorScoringConfigHistoryOrderFieldScoringMode
	default:
		return fmt.Errorf("%s is not a valid VendorScoringConfigHistoryOrderField", str)
	}
	return nil
}

// VendorScoringConfigHistoryOrderField defines the ordering field of VendorScoringConfigHistory.
type VendorScoringConfigHistoryOrderField struct {
	// Value extracts the ordering value from the given VendorScoringConfigHistory.
	Value    func(*VendorScoringConfigHistory) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) vendorscoringconfighistory.OrderOption
	toCursor func(*VendorScoringConfigHistory) Cursor
}

// VendorScoringConfigHistoryOrder defines the ordering of VendorScoringConfigHistory.
type VendorScoringConfigHistoryOrder struct {
	Direction OrderDirection                        `json:"direction"`
	Field     *VendorScoringConfigHistoryOrderField `json:"field"`
}

// DefaultVendorScoringConfigHistoryOrder is the default ordering of VendorScoringConfigHistory.
var DefaultVendorScoringConfigHistoryOrder = &VendorScoringConfigHistoryOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &VendorScoringConfigHistoryOrderField{
		...
		toTerm: vendorscoringconfighistory.ByID,
		toCursor: func(_m *VendorScoringConfigHistory) Cursor {
			return Cursor{ID: _m.ID}
		},
	},
...
// ToEdge converts VendorScoringConfigHistory into VendorScoringConfigHistoryEdge.
func (_m *VendorScoringConfigHistory) ToEdge(order *VendorScoringConfigHistoryOrder) *VendorScoringConfigHistoryEdge {
	if order == nil {
		order = DefaultVendorScoringConfigHistoryOrder
	}
	return &VendorScoringConfigHistoryEdge{
		Node:   _m,
		Cursor: order.Field.toCursor(_m),
	}
}
...
func (_m *VulnerabilityHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*VulnerabilityHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *WorkflowAssignmentHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*WorkflowAssignmentHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *WorkflowAssignmentTargetHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*WorkflowAssignmentTargetHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
...
func (_m *WorkflowDefinitionHistoryQuery) Paginate(
	...
	_m = pager.applyOrder(_m)
	var nodes []*WorkflowDefinitionHistory
	if limit == 0 {
		if nodes, err = _m.All(ctx); err != nil {
			return nil, err
		}
