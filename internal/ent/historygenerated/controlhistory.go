...
func (_m *ControlHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case controlhistory.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
...
func (_m *ControlHistory) Unwrap() *ControlHistory {
	...
	if !ok {
		panic("historygenerated: ControlHistory is not a transactional entity")
	}
