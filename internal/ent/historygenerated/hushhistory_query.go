...
func (_q *HushHistoryQuery) prepareQuery(ctx context.Context) error {
	...
	if hushhistory.Policy == nil {
		return errors.New("historygenerated: uninitialized hushhistory.Policy (forgotten import historygenerated/runtime?)")
	}
...
func (hhq *HushHistoryQuery) CountIDs(ctx context.Context) (int, error) {
	...
	ids, err := hhq.IDs(ctx)
	if err != nil {
		logx.ErrorEvent(ctx, err).Str("query_type", "HushHistory").Str("operation", "count_ids").Msg("CountIDs: IDs() failed")
