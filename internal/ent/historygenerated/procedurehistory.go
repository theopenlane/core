...
func (_m *ProcedureHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case procedurehistory.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case procedurehistory.FieldManagementMode:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case procedurehistory.FieldDetailsJSON:
			if value, ok := values[i].(*[]byte); !ok {
...
func (_m *ProcedureHistory) Unwrap() *ProcedureHistory {
	...
	if !ok {
		panic("historygenerated: ProcedureHistory is not a transactional entity")
	}
