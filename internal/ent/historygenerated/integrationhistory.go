...
func (_m *IntegrationHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case integrationhistory.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case integrationhistory.FieldPlatformID:
			if value, ok := values[i].(*sql.NullString); !ok {
