...
func (*ContactHistory) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case contacthistory.FieldObservedAt:
...
func (_m *ContactHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case contacthistory.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case contacthistory.FieldExternalID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field external_id", values[i])
			} else if value.Valid {
				_m.ExternalID = value.String
			}
		case contacthistory.FieldIntegrationID:
			if value, ok := values[i].(*sql.NullString); !ok {
