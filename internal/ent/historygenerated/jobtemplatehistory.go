...
func (_m *JobTemplateHistory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case jobtemplatehistory.FieldUpdatedByImpersonator:
			if value, ok := values[i].(*sql.NullString); !ok {
...
func (_m *JobTemplateHistory) Unwrap() *JobTemplateHistory {
	...
	if !ok {
		panic("historygenerated: JobTemplateHistory is not a transactional entity")
	}
