...
func (TemplateResponder) Fields() []ent.Field {
	return []ent.Field{
		field.String("template_id").
			Comment("the template id associated with this responder"),
		...
		field.String("document_data_id").
			Comment("the document data id that stores the response after completion").
			Optional(),
	}
}
...
func (TemplateResponder) Indexes() []ent.Index {
	return []ent.Index{
		...
		index.Fields("template_id", "status"),

		index.Fields("recipient_email", ownerFieldName),

		index.Fields("recipient_email", "template_id").
			Unique(),
	}
}
