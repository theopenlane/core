...
func handleFileBackup(ctx gala.HandlerContext, payload eventqueue.MutationGalaPayload) error {
	...
	// when reads are served from the backup the source storage is presumed lost, so replicating
	// from it would only fail, burn attempts and permanently mark files exhausted
	if _, ok := svc.ReadFromBackupFor(source); ok {
		return nil
	}
