...
func TestAnyRuntimeInterestedMatchesConcernTopics(t *testing.T) {
	workflowRuntime, err := gala.NewGala(context.Background(), gala.Config{DispatchMode: gala.DispatchModeInMemory, WorkerCount: 3})
	if err != nil {
		t.Fatalf("failed to create workflow runtime: %v", err)
	}

	notificationRuntime, err := gala.NewGala(context.Background(), gala.Config{DispatchMode: gala.DispatchModeInMemory, WorkerCount: 3})
	if err != nil {
