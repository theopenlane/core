...
// func (suite *HandlerTestSuite) createTrustCenterWithSlug(ctx context.Context, slug string) *ent.TrustCenter {
// 	t := suite.T()

// 	trustCenter, err := suite.db.TrustCenter.Create().
// 		SetSlug(slug).
// 		SetOwnerID(testUser1.OrganizationID).
// 		Save(ctx)
// 	require.NoError(t, err)
...
// func (suite *HandlerTestSuite) createCustomDomain(ctx context.Context, cnameRecord string) *ent.CustomDomain {
// 	t := suite.T()

// 	// First create a mappable domain
// 	mappableDomain, err := suite.db.MappableDomain.Create().
// 		SetName(gofakeit.DomainName()).
// 		SetZoneID(gofakeit.UUID()).
// 		Save(ctx)
// 	require.NoError(t, err)

// 	customDomain, err := suite.db.CustomDomain.Create().
// 		SetCnameRecord(cnameRecord).
// 		SetMappableDomainID(mappableDomain.ID).
// 		SetOwnerID(testUser1.OrganizationID).
// 		Save(ctx)
// 	require.NoError(t, err)
...
// func (suite *HandlerTestSuite) createTrustCenterWithCustomDomain(ctx context.Context, customDomainID string) *ent.TrustCenter {
// 	t := suite.T()

// 	trustCenter, err := suite.db.TrustCenter.Create().
// 		SetSlug(gofakeit.Word()).
// 		SetOwnerID(testUser1.OrganizationID).
// 		SetCustomDomainID(customDomainID).
// 		Save(ctx)
// 	require.NoError(t, err)
...
// func (suite *HandlerTestSuite) cleanupTrustCenter(ctx context.Context, id string) {
// 	t := suite.T()

// 	err := suite.db.TrustCenter.DeleteOneID(id).Exec(ctx)
// 	require.NoError(t, err)
// }

// func (suite *HandlerTestSuite) cleanupCustomDomain(ctx context.Context, id string) {
// 	// Get the custom domain to find the mappable domain
// 	customDomain, err := suite.db.CustomDomain.Get(ctx, id)
// 	if err == nil {
// 		// Delete the mappable domain first
// 		_ = suite.db.MappableDomain.DeleteOneID(customDomain.MappableDomainID).Exec(ctx)
// 	}

// 	// Delete the custom domain
// 	_ = suite.db.CustomDomain.DeleteOneID(id).Exec(ctx)
// }
