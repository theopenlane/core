...
func WithKeyDirWatcher(dir string) ServerOption {
	return newApplyFunc(func(s *ServerOptions) {
		...
		if _, err := os.Stat(dir); err != nil {
			log.Error().Err(err).Msg("key directory not found")
			return
		}
...
// WithKeyDirWatcherFromEnv calls WithKeyDirWatcher if CORE_AUTH_TOKEN_KEYDIR is set.
func WithKeyDirWatcherFromEnv() ServerOption {
	dir := os.Getenv("CORE_AUTH_TOKEN_KEYDIR")
	if dir == "" {
		return newApplyFunc(func(*ServerOptions) {})
	}

	return WithKeyDirWatcher(dir)
}
