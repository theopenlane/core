...
// backupFile returns the backup provider and a rewritten file pointing at the replica when reads
// for the file's source provider are configured to be served from its backup. This is a routing
// decision made before any I/O, not a retry, so the source provider is never contacted. Files
// without a usable replica fall through to the source provider as normal
func (s *Service) backupFile(ctx context.Context, file *storagetypes.File, operation string) (storage.Provider, *storagetypes.File, bool) {
	if file == nil {
	...
	provider, ok := s.ReadFromBackupFor(source)
	if !ok {
		return nil, nil, false
	}

	if file.BackupLocation == nil || file.BackupLocation.Key == "" {
		// reads come from the backup but this file was never replicated, so the source provider is
		// the only place it can be served from
		...
		logx.FromContext(ctx).Warn().
			...
			Msg("no backup replica for file; falling through to source provider")

		return nil, nil, false
	}

	replica := replicaFile(file)

	metrics.RecordStorageBackupRead(string(source), string(replica.ProviderType), operation)

	return provider, replica, true
}
...
func (s *Service) Download(ctx context.Context, provider storage.Provider, file *storagetypes.File, opts *storage.DownloadOptions) (*storage.DownloadedMetadata, error) {
	if provider == nil {
		if backupProvider, replica, ok := s.backupFile(ctx, file, backupOpDownload); ok {
			return s.objectService.Download(ctx, backupProvider, replica, opts)
		}
...
func (s *Service) GetPresignedURL(ctx context.Context, file *storagetypes.File, duration time.Duration) (string, error) {
	...
	if backupProvider, replica, ok := s.backupFile(ctx, file, backupOpPresignedURL); ok {
		return s.objectService.GetPresignedURL(ctx, backupProvider, replica, opts)
	}
...
func (s *Service) DeleteBackup(ctx context.Context, file *storagetypes.File, opts *storagetypes.DeleteFileOptions) error {
	...
	err := s.objectService.Delete(ctx, target.Provider, replica, opts)
...
func (s *Service) Exists(ctx context.Context, file *storagetypes.File) (bool, error) {
	if backupProvider, replica, ok := s.backupFile(ctx, file, backupOpExists); ok {
		return backupProvider.Exists(ctx, replica)
	}
