...
func (r *subscriptionResolver) handleNotificationSubscription(ctx context.Context) (<-chan *generated.Notification, error) {
	...
	// anonymous callers (trust center, questionnaire) are scoped to an org but are not members of
	// it, so they must never join the org-wide fan-out
	var orgID string
	...
	if orgID == "" {
		logx.FromContext(ctx).Warn().Str("user_id", userID).Msg("notification subscription: no authorized organization, org-wide notifications will not be delivered")
	}
