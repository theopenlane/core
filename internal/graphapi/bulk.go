...
func (r *mutationResolver) bulkUpdateOrganizationSetting(ctx context.Context, ids []string, input generated.UpdateOrganizationSettingInput) (*model.OrganizationSettingBulkUpdatePayload, error) {
	...
	for _, id := range ids {
		...
		// setup update request
		updatedEntity, err := existing.Update().SetInput(input).AppendTags(input.AppendTags).AppendDomains(input.AppendDomains).AppendAllowedEmailDomains(input.AppendAllowedEmailDomains).AppendIdentityProviderExemptDomains(input.AppendIdentityProviderExemptDomains).Save(ctx)
		if err != nil {
...
func (r *mutationResolver) bulkUpdateCSVOrganizationSetting(ctx context.Context, inputs []*csvgenerated.OrganizationSettingCSVUpdateInput) (*model.OrganizationSettingBulkUpdatePayload, error) {
	...
	for _, input := range inputs {
		...
		// setup update request with this row's input values
		updatedEntity, err := existing.Update().SetInput(input.Input).AppendTags(input.Input.AppendTags).AppendDomains(input.Input.AppendDomains).AppendAllowedEmailDomains(input.Input.AppendAllowedEmailDomains).AppendIdentityProviderExemptDomains(input.Input.AppendIdentityProviderExemptDomains).Save(ctx)
		if err != nil {
