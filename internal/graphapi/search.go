...
func adminSearchControls(ctx context.Context, query string) ([]*generated.Control, error) {
	return withTransactionalMutation(ctx).Control.Query().Where(
		control.Or(
			...
			func(s *sql.Selector) {
				likeQuery := "%" + query + "%"
				s.Where(sql.ExprP("(mappedcategories)::text LIKE $12", likeQuery)) // search by MappedCategories
			},
			func(s *sql.Selector) {
				likeQuery := "%" + query + "%"
				s.Where(sql.ExprP("(assessmentobjectives)::text LIKE $13", likeQuery)) // search by AssessmentObjectives
			},
			func(s *sql.Selector) {
				likeQuery := "%" + query + "%"
				s.Where(sql.ExprP("(assessmentmethods)::text LIKE $14", likeQuery)) // search by AssessmentMethods
			},
			func(s *sql.Selector) {
				likeQuery := "%" + query + "%"
				s.Where(sql.ExprP("(controlquestions)::text LIKE $15", likeQuery)) // search by ControlQuestions
			},
			func(s *sql.Selector) {
				likeQuery := "%" + query + "%"
				s.Where(sql.ExprP("(implementationguidance)::text LIKE $16", likeQuery)) // search by ImplementationGuidance
			},
			func(s *sql.Selector) {
				likeQuery := "%" + query + "%"
				s.Where(sql.ExprP("(exampleevidence)::text LIKE $17", likeQuery)) // search by ExampleEvidence
			},
			func(s *sql.Selector) {
				likeQuery := "%" + query + "%"
