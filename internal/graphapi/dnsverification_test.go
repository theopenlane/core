	...
	testCases := []struct {
	...
	}{
		{
			...
			request: openlaneclient.CreateDNSVerificationInput{
				...
				SslTxtRecord:         "_acme-challenge.example.com",
				SslTxtValue:          "test-ssl-value",
				OwnerID:              lo.ToPtr(testUser1.OrganizationID),
			},
			client: suite.client.api,
			ctx:    systemAdminUser.UserCtx,
		},
		{
			...
			request: openlaneclient.CreateDNSVerificationInput{
				...
				SslTxtRecord:         "_acme-challenge.unauthorized.example.com",
				SslTxtValue:          "test-ssl-value-unauthorized",
				OwnerID:              lo.ToPtr(testUser1.OrganizationID),
			},
			client:      suite.client.api,
			ctx:         testUser1.UserCtx,
		...
		},
		{
			...
			request: openlaneclient.CreateDNSVerificationInput{
				...
				SslTxtRecord: "_acme-challenge.missing.example.com",
				SslTxtValue:  "test-ssl-value-missing",
				OwnerID:      lo.ToPtr(testUser1.OrganizationID),
			},
			client:      suite.client.api,
			ctx:         systemAdminUser.UserCtx,
...
func TestUpdateDNSVerification(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			ctx:     systemAdminUser.UserCtx,
			updateInput: openlaneclient.UpdateDNSVerificationInput{
				SslCertStatus:               lo.ToPtr(enums.CustomDomainStatusVerified),
				DNSVerificationStatus:       lo.ToPtr(enums.CustomDomainStatusVerified),
				SslCertStatusReason:         lo.ToPtr("all good!"),
