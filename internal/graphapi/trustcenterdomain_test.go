...
func TestMutationCreateTrustCenterDomain(t *testing.T) {
	...
	t.Run("normalizes cname record input", func(t *testing.T) {
		...
		assert.NilError(t, err)
		assert.Check(t, is.Equal(testutils.TrustCenterPreviewCnameTarget, previewMappableDomain.Name))
		assert.Check(t, is.Equal(testutils.TrustCenterPreviewZoneID, previewMappableDomain.ZoneID))
