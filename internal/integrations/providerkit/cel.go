...
func envelopeToVars(envelope types.AlertEnvelope) map[string]any {
	var payload any
...
func EvalFilter(ctx context.Context, expr string, envelope types.AlertEnvelope) (bool, error) {
	if expr == "" {
		return true, nil
	}
...
func EvalMap(ctx context.Context, expr string, envelope types.AlertEnvelope) (json.RawMessage, error) {
	if expr == "" {
