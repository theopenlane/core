...
func RegisterScheduledListener[T any](cfg ScheduledListenerConfig[T]) error {
	...
	_, err := gala.RegisterListeners(cfg.Runtime.Registry(), gala.Definition[T]{
		...
		Handle: func(ctx gala.HandlerContext, envelope T) error {
			...
			if execErr != nil {
				if cfg.ShouldCancel != nil && cfg.ShouldCancel(ctx.Context, envelope, execErr) {
			...
			effectiveSchedule := cfg.Schedule
			if cfg.ScheduleOverride != nil {
				if override := cfg.ScheduleOverride(envelope); override != nil {
					effectiveSchedule = *override
				}
			}

			next := effectiveSchedule.Next(cfg.State(envelope), delta, execErr)
