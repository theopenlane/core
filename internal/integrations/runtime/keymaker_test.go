...
func TestMapKeymakerPersistAuthError(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:        "installation id required",
			err:         ErrInstallationIDRequired,
			expectedErr: keymaker.ErrInstallationIDRequired,
		},
		{
			name:        "installation not found",
			err:         ErrInstallationNotFound,
			expectedErr: keymaker.ErrInstallationNotFound,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := mapKeymakerPersistAuthError(tt.err)
			switch {
