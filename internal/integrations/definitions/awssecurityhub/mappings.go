...
var mapExprFinding = providerkit.CelMapExpr([]providerkit.CelMapEntry{
	...
	{Key: integrationgenerated.IntegrationMappingFindingCategory, Expr: `'Types' in payload && payload.Types != null && size(payload.Types) > 0 ? payload.Types[0] : ""`},
	{Key: integrationgenerated.IntegrationMappingFindingResourceName, Expr: `'Resources' in payload && payload.Resources != null && size(payload.Resources) > 0 && payload.Resources[0] != null ? ('ApplicationName' in payload.Resources[0] && payload.Resources[0].ApplicationName != null ? payload.Resources[0].ApplicationName : ('Id' in payload.Resources[0] ? payload.Resources[0].Id : "")) : ""`},
	{Key: integrationgenerated.IntegrationMappingFindingTargets, Expr: `'Resources' in payload && payload.Resources != null && size(payload.Resources) > 0 ? payload.Resources.filter(r, r != null && 'Id' in r).map(r, r.Id) : []`},
	{Key: integrationgenerated.IntegrationMappingFindingTargetDetails, Expr: `'Resources' in payload && payload.Resources != null && size(payload.Resources) > 0 ? indexBy(payload.Resources.filter(r, r != null && 'Id' in r), "Id") : {}`},
	{Key: integrationgenerated.IntegrationMappingFindingExternalURI, Expr: `'SourceUrl' in payload ? payload.SourceUrl : ""`},
	{Key: integrationgenerated.IntegrationMappingFindingExternalOwnerID, Expr: `'Resources' in payload && payload.Resources != null && size(payload.Resources) > 0 && payload.Resources[0] != null && 'Id' in payload.Resources[0] && payload.Resources[0].Id != "" ? payload.Resources[0].Id : 'AwsAccountId' in payload ? payload.AwsAccountId : ""`},
...
var mapExprVulnerability = providerkit.CelMapExpr([]providerkit.CelMapEntry{
	...
	{Key: integrationgenerated.IntegrationMappingVulnerabilityCveID, Expr: `'Vulnerabilities' in payload && payload.Vulnerabilities != null && size(payload.Vulnerabilities) > 0 && payload.Vulnerabilities[0] != null && 'Id' in payload.Vulnerabilities[0] ? payload.Vulnerabilities[0].Id : ""`},
	{Key: integrationgenerated.IntegrationMappingVulnerabilityExternalOwnerID, Expr: `'Resources' in payload && payload.Resources != null && size(payload.Resources) > 0 && payload.Resources[0] != null && 'Id' in payload.Resources[0] && payload.Resources[0].Id != "" ? payload.Resources[0].Id : 'AwsAccountId' in payload ? payload.AwsAccountId : ""`},
	{Key: integrationgenerated.IntegrationMappingVulnerabilityCategory, Expr: `'Types' in payload && payload.Types != null && size(payload.Types) > 0 ? payload.Types[0] : ""`},
	...
	{Key: integrationgenerated.IntegrationMappingVulnerabilityFixAvailable, Expr: `'Vulnerabilities' in payload && payload.Vulnerabilities != null && size(payload.Vulnerabilities) > 0 && payload.Vulnerabilities[0] != null && 'FixAvailable' in payload.Vulnerabilities[0] ? payload.Vulnerabilities[0].FixAvailable == "YES" : false`},
	...
	{Key: integrationgenerated.IntegrationMappingVulnerabilityReferences, Expr: `'Vulnerabilities' in payload && payload.Vulnerabilities != null && size(payload.Vulnerabilities) > 0 && payload.Vulnerabilities[0] != null && 'ReferenceUrls' in payload.Vulnerabilities[0] ? payload.Vulnerabilities[0].ReferenceUrls : []`},
