...
var mapExprFinding = providerkit.CelMapExpr([]providerkit.CelMapEntry{
	...
	{Key: integrationgenerated.IntegrationMappingVulnerabilityVulnerabilityStatusName, Expr: `'Workflow' in payload && payload.Workflow != null && 'Status' in payload.Workflow && payload.Workflow.Status != "" ? payload.Workflow.Status : ('RecordState' in payload ? payload.RecordState : "")`},
...
var mapExprAssessment = providerkit.CelMapExpr([]providerkit.CelMapEntry{
	...
	{Key: integrationgenerated.IntegrationMappingFindingFindingStatusName, Expr: `'status' in payload ? payload.status : ""`},
