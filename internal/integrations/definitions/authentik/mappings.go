...
var mapExprDirectoryAccount = providerkit.CelMapExpr([]providerkit.CelMapEntry{
	...
	{Key: integrationgenerated.IntegrationMappingDirectoryAccountAccountType, Expr: `dyn(
  'type' in payload && payload.type != null ? (
    payload.type == "internal" ? "USER" : (
    payload.type == "external" ? "GUEST" : (
    payload.type == "service_account" ? "SERVICE" : (
    payload.type == "internal_service_account" ? "SERVICE" : "USER"
  )))) : "USER")`},
...
var mapExprDirectoryGroup = providerkit.CelMapExpr([]providerkit.CelMapEntry{
	...
	{Key: integrationgenerated.IntegrationMappingDirectoryGroupClassification, Expr: `dyn('is_superuser' in payload && payload.is_superuser ? "SECURITY" : "TEAM")`},
