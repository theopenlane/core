...
var mapExprDirectoryAccount = providerkit.CelMapExpr([]providerkit.CelMapEntry{
	...
	{Key: entityops.InputKeyDirectoryAccountDisplayName, Expr: `'human' in payload ? ('profile' in payload.human && 'display_name' in payload.human.profile && payload.human.profile.display_name != "" ? payload.human.profile.display_name : ('username' in payload ? payload.username : "")) : ('machine' in payload && 'name' in payload.machine ? payload.machine.name : ('username' in payload ? payload.username : ""))`},
	...
	{Key: entityops.InputKeyDirectoryAccountPhoneNumber, Expr: `'human' in payload && 'phone' in payload.human && 'phone' in payload.human.phone ? payload.human.phone.phone : ""`},
	{Key: entityops.InputKeyDirectoryAccountAvatarRemoteURL, Expr: `'human' in payload && 'profile' in payload.human && 'avatar_url' in payload.human.profile ? payload.human.profile.avatar_url : ""`},
