...
var (
	...
	mapExprDependabotPoll = buildPollMappingExpr(githubAlertTypeDependabot, `"github:" + resource + ":dependabot:" + ('Number' in payload && payload.Number != 0 ? string(payload.Number) : ('SecurityVulnerability' in payload && 'Advisory' in payload.SecurityVulnerability && 'GHSAID' in payload.SecurityVulnerability.Advisory && payload.SecurityVulnerability.Advisory.GHSAID != "" ? payload.SecurityVulnerability.Advisory.GHSAID : "unknown"))`, []providerkit.CelMapEntry{
		...
		{Key: integrationgenerated.IntegrationMappingVulnerabilityScore, Expr: `'SecurityVulnerability' in payload && 'Advisory' in payload.SecurityVulnerability && 'CvssSeverities' in payload.SecurityVulnerability.Advisory && 'CvssV4' in payload.SecurityVulnerability.Advisory.CvssSeverities && payload.SecurityVulnerability.Advisory.CvssSeverities.CvssV4 != null ? payload.SecurityVulnerability.Advisory.CvssSeverities.CvssV4.Score : null`},
		{Key: integrationgenerated.IntegrationMappingVulnerabilityVector, Expr: `'SecurityVulnerability' in payload && 'Advisory' in payload.SecurityVulnerability && 'CvssSeverities' in payload.SecurityVulnerability.Advisory && 'CvssV4' in payload.SecurityVulnerability.Advisory.CvssSeverities && payload.SecurityVulnerability.Advisory.CvssSeverities.CvssV4 != null ? payload.SecurityVulnerability.Advisory.CvssSeverities.CvssV4.VectorString : ""`},
