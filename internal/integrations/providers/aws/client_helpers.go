...
// AWSClientConstructor is a function that creates a typed AWS client from an aws.Config
type AWSClientConstructor[T any] func(cfg awssdk.Config) T

// buildAWSClient builds a typed AWS client from stored credentials using the provided constructor function
func buildAWSClient[T any](ctx context.Context, payload types.CredentialPayload, constructor AWSClientConstructor[T]) (T, auth.AWSMetadata, error) {
	var zero T
...
func resolveAWSClient[T any](ctx context.Context, input types.OperationInput, constructor AWSClientConstructor[T]) (T, auth.AWSMetadata, error) {
