...
import (
	...
	integrationtypes "github.com/theopenlane/core/internal/integrationsv2/types"
...
func TestRegistryRegisterAndResolveDefinition(t *testing.T) {
	...
	bySlug, ok := reg.BySlug(" GitHub_App ")
	if !ok {
		t.Fatalf("BySlug() did not find %q", definition.Spec.Slug)
	}

	if bySlug.Spec.ID != definition.Spec.ID {
		t.Fatalf("BySlug() id = %q, want %q", bySlug.Spec.ID, definition.Spec.ID)
	}
