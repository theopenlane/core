...
func tableOutput(out []openlaneclient.Control) {
	...
	writer := tables.NewTableWriter(command.OutOrStdout(), "ID", "Name", "Description", "Status", "ControlType")
	...
	for _, i := range out {
		...
		writer.AddRow(i.ID, i.RefCode, *i.Description, *i.Status, *i.ControlType)
	}
