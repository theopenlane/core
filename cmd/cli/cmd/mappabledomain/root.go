...
func tableOutput(out interface{}) {
	...
	writer := tables.NewTableWriter(command.OutOrStdout(), "ID", "Name", "Created At", "Created By")

	switch v := out.(type) {
	case []map[string]interface{}:
		for _, i := range v {
			writer.AddRow(i["ID"], i["Name"], i["CreatedAt"], i["CreatedBy"])
		}
	case openlaneclient.MappableDomain:
		writer.AddRow(v.ID, v.Name, v.CreatedAt, v.CreatedBy)
	case []openlaneclient.MappableDomain:
		for _, i := range v {
			cb := ""
			if i.CreatedBy != nil {
			...
			writer.AddRow(i.ID, i.Name, i.CreatedAt, cb)
		}
	}
