...
func init() {
	...
	updateCmd.Flags().StringP("status", "s", "", "new status (e.g., ACTIVE, PENDING)")
}

// updateValidation validates the required fields for the command
func updateValidation() (id string, input openlaneclient.UpdateCustomDomainInput, err error) {
	...
	if status != "" {
		...
		input.Status = enums.ToCustomDomainStatus(status)
	}
	return id, input, nil
}
