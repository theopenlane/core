...
func tableOutput(out []openlaneclient.CustomDomain) {
	...
	writer := tables.NewTableWriter(command.OutOrStdout(), "ID", "CNAME", "TXT", "TXT Secret", "Status", "Created At")
	for _, i := range out {
		writer.AddRow(i.ID, i.CnameRecord, fmt.Sprintf("%s.%s", i.TxtRecordSubdomain, i.CnameRecord), i.TxtRecordValue, i.Status, i.CreatedAt)
	}
