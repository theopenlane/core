...
func tableOutput(out []openlaneclient.Subcontrol) {
	...
	writer := tables.NewTableWriter(command.OutOrStdout(), "ID", "Name", "Description", "Status", "SubcontrolType", "Version", "SubcontrolNumber", "Family", "Class", "Source", "MappedFrameworks", "ImplementationEvidence", "ImplementationStatus", "ImplementationVerification", "Programs")
	...
	for _, i := range out {
		...
		writer.AddRow(i.ID, i.Name, *i.Description, *i.Status, *i.SubcontrolType, *i.Version, *i.SubcontrolNumber, *i.Family, *i.Class, *i.Source, i.MappedFrameworks, *i.ImplementationEvidence, *i.ImplementationStatus, *i.ImplementationVerification, programs)
	}
