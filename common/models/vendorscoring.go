...
// to the category prefix. User-provided keys on custom entries are always overwritten
// to prevent collisions with CAIQ-sourced defaults
func (v *VendorScoringQuestionsConfig) AssignCustomKeys() {
	...
	for i := range v.Custom {
		if isCustomGeneratedKey(v.Custom[i].Key) {
			continue
		}
