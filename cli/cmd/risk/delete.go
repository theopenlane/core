...
func init() {
	...
	deleteCmd.Flags().StringP("id", "i", "", "risk id to delete")
}
...
func deleteValidation() (string, error) {
	...
	if id == "" {
		return "", cmd.NewRequiredFieldMissingError("risk id")
	}
...
func delete(ctx context.Context) error {
	...
	o, err := client.DeleteRisk(ctx, id)
	cobra.CheckErr(err)
