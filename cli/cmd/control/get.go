...
func get(ctx context.Context) error {
	...
	var order *graphclient.ControlOrder
	if cmd.OrderBy != nil && cmd.OrderDirection != nil {
		order = &graphclient.ControlOrder{
			Direction: graphclient.OrderDirection(strings.ToUpper(*cmd.OrderDirection)),
			Field:     graphclient.ControlOrderField(*cmd.OrderBy),
		}
	}

	// if a ref code is provided, filter on that control
	if refCode != "" {
		o, err := client.GetControls(ctx, cmd.First, cmd.Last, cmd.After, cmd.Before, &graphclient.ControlWhereInput{
			RefCode: &refCode,
		}, []*graphclient.ControlOrder{order})
		cobra.CheckErr(err)
	...
	o, err := client.GetAllControls(ctx, cmd.First, cmd.Last, cmd.After, cmd.Before, []*graphclient.ControlOrder{order})
	cobra.CheckErr(err)
