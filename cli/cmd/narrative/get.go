...
func get(ctx context.Context) error {
	...
	var order *graphclient.NarrativeOrder
	if cmd.OrderBy != nil && cmd.OrderDirection != nil {
		order = &graphclient.NarrativeOrder{
			Direction: graphclient.OrderDirection(strings.ToUpper(*cmd.OrderDirection)),
			Field:     graphclient.NarrativeOrderField(*cmd.OrderBy),
		}
	}

	// get all will be filtered for the authorized organization(s)
	o, err := client.GetAllNarratives(ctx, cmd.First, cmd.Last, cmd.After, cmd.Before, []*graphclient.NarrativeOrder{order})
	cobra.CheckErr(err)
