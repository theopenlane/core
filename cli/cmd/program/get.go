...
func get(ctx context.Context) error {
	...
	var order *graphclient.ProgramOrder
	if cmd.OrderBy != nil && cmd.OrderDirection != nil {
		order = &graphclient.ProgramOrder{
			Direction: graphclient.OrderDirection(strings.ToUpper(*cmd.OrderDirection)),
			Field:     graphclient.ProgramOrderField(*cmd.OrderBy),
		}
	}

	// get all will be filtered for the authorized organization(s)
	o, err := client.GetAllPrograms(ctx, cmd.First, cmd.Last, cmd.After, cmd.Before, []*graphclient.ProgramOrder{order})
	cobra.CheckErr(err)
