...
func get(ctx context.Context) error {
	...
	var order *graphclient.JobTemplateOrder
	if cmd.OrderBy != nil && cmd.OrderDirection != nil {
		order = &graphclient.JobTemplateOrder{
			Direction: graphclient.OrderDirection(strings.ToUpper(*cmd.OrderDirection)),
			Field:     graphclient.JobTemplateOrderField(*cmd.OrderBy),
		}
	}

	// get all will be filtered for the authorized organization(s)
	o, err := client.GetAllJobTemplates(ctx, cmd.First, cmd.Last, cmd.After, cmd.Before, []*graphclient.JobTemplateOrder{order})
	cobra.CheckErr(err)
